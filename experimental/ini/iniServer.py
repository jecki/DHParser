...
"""iniServer.py - starts a server (if not already running) for the
                    compilation of ini
"""
...
class iniLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "ini-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = iniCPUBoundTasks(self.lsp_data)
        self.blocking = iniBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from iniParser import batch_process
        args = argstr.split(' ')
    ...
    #     from iniParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new iniServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    # from tst_ini_grammar import recompile_grammar
    # recompile_grammar(os.path.join(scriptpath, 'ini.ebnf'), force=False)
    from DHParser.dsl import recompile_grammar
    if not recompile_grammar(grammar_src, force=False,
                             notify=lambda: print('recompiling ' + grammar_src)):
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('ini_ebnf_ERRORS.txt', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    ini_lsp = iniLanguageServerProtocol()
    lsp_table = ini_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', ini_lsp.simply_compile)
    ini_server = Server(rpc_functions=lsp_table,
                        cpu_bound=ini_lsp.cpu_bound.lsp_table.keys(),
                        blocking=ini_lsp.blocking.lsp_table.keys(),
                        connection_callback=ini_lsp.connect,
                        server_name="iniServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        ini_server.echo_log = True if port >= 0 and host else False
        msg = ini_server.start_logging(log_path.strip('" \''))
        if ini_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        ini_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            ini_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing ini-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python iniServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python iniServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python iniServer.py --stream\n'
             + '    python iniServer.py --stopserver\n'
             + '    python iniServer.py --status\n'
             + '    python iniServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python iniServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
