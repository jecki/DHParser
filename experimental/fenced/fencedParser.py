...
from DHParser.toolkit import re, INFINITE
...
def fencedTokenizer(original_text) -> Tuple[str, List[Error]]:
...
def preprocessor_factory() -> PreprocessorFunc:
    # below, the second parameter must always be the same as fencedGrammar.COMMENT__!
    find_next_include = gen_find_include_func(RE_INCLUDE, NEVER_MATCH_PATTERN)
    include_prep = partial(preprocess_includes, find_next_include=find_next_include)
    tokenizing_prep = make_preprocessor(fencedTokenizer)
...
class fencedGrammar(Grammar):
    r"""Parser for a fenced source file.
    """
    source_hash__ = "f90dbff4cdffc612e56ae1115127fae5"
    ...
    fence = Capture(RegExp('\\~+'))
    no_fence = Series(NegativeLookahead(Retrieve(fence)), RegExp('\\~+'), dwsp__)
    text = Series(RegExp('[^\\~\\n]+'), dwsp__)
    fenced = Series(fence, dwsp__, ZeroOrMore(Alternative(text, no_fence)), Pop(fence), dwsp__)
    document = Series(dwsp__, ZeroOrMore(Alternative(text, fenced)), EOF)
    root__ = document
    

_raw_grammar = ThreadLocalSingletonFactory(fencedGrammar, ident=1)

def get_grammar() -> fencedGrammar:
...
def parse_fenced(document, start_parser = "root_parser__", *, complete_match=True):
...
fenced_AST_transformation_table = {
    # AST Transformations for the fenced-grammar
    # "<": flatten
...
def fencedTransformer() -> TransformerFunc:
    """Creates a transformation function that does not share state with other
    threads or processes."""
    return partial(traverse, transformation_table=fenced_AST_transformation_table.copy())


get_transformer = ThreadLocalSingletonFactory(fencedTransformer, ident=1)


def transform_fenced(cst):
...
class fencedCompiler(Compiler):
    """Compiler for the abstract-syntax-tree of a fenced source file.
    """

    def __init__(self):
        super(fencedCompiler, self).__init__()
    ...
    # def on_fenced(self, node):
    #     return node

    # def on_fence(self, node):
    #     return node
...
get_compiler = ThreadLocalSingletonFactory(fencedCompiler, ident=1)


def compile_fenced(ast):
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Parses a fenced-file and shows its syntax-tree.")
