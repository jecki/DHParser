...
from DHParser import start_logging, suspend_logging, resume_logging, is_filename, load_if_file, \
    ...
    gen_neutral_srcmap_func, make_preprocessor, chain_preprocessors, IncludeInfo
...
def preprocessor_factory() -> PreprocessorFunc:
    generic_find_next_include = gen_find_include_func(RE_INCLUDE, LaTeXGrammar.comment_rx__)

    def find_next_include(text: str, start: int) -> IncludeInfo:
        nonlocal generic_find_next_include
        begin, length, filename = generic_find_next_include(text, start)
        return IncludeInfo(begin, length, filename + '.tex')
...
class LaTeXGrammar(Grammar):
    ...
    source_hash__ = "12ac92f84b372261c6fe136a3f112691"
    ...
    LF = Alternative(Series(NEW_LINE, ZeroOrMore(Series(comment__, whitespace__))), EOF)
    _LETTERS = RegExp('\\w+')
    CHARS = RegExp('[^\\\\%$&\\{\\}\\[\\]\\s\\n]+')
    LINE = RegExp('[^\\\\%$&\\{\\}\\[\\]\\n]+')
    TEXT_NOPAR = RegExp('(?:[^\\\\%$&\\{\\}\\[\\]\\(\\)\\n]+(?:\\n(?![ \\t]*\\n))?)+')
    ...
    _structure_name = Drop(Alternative(Drop(Text("subsection")), Drop(Text("section")), Drop(Text("chapter")), Drop(Text("subsubsection")), Drop(Text("paragraph")), Drop(Text("subparagraph")), Drop(Text("item")), Drop(Text("RWch")), Drop(Text("RWteil")), Drop(Text("RWpar"))))
    _env_name = Drop(Alternative(Drop(Text("enumerate")), Drop(Text("itemize")), Drop(Text("description")), Drop(Text("figure")), Drop(Text("quote")), Drop(Text("quotation")), Drop(Text("tabular")), Drop(Series(Drop(Text("displaymath")), Drop(Option(Drop(Text("*")))))), Drop(Series(Drop(Text("equation")), Drop(Option(Drop(Text("*")))))), Drop(Series(Drop(Text("eqnarray")), Drop(Option(Drop(Text("*")))))), Drop(Text("compactenum")), Drop(Text("aufza")), Drop(Text("aufzb"))))
    blockcmd = Series(BACKSLASH, Alternative(Series(Alternative(Series(Drop(Text("begin{")), dwsp__), Series(Drop(Text("end{")), dwsp__)), _env_name, Series(Drop(Text("}")), dwsp__)), Series(_structure_name, Lookahead(Series(Drop(Text("{")), dwsp__))), Drop(Text("[")), Drop(Text("]"))))
    no_command = Alternative(Series(Drop(Text("\\begin{")), dwsp__), Series(Drop(Text("\\end{")), dwsp__), Series(BACKSLASH, _structure_name, Lookahead(Series(Drop(Text("{")), dwsp__))))
    text = Series(OneOrMore(Alternative(TEXT, special)), ZeroOrMore(Series(S, OneOrMore(Alternative(TEXT, special)))))
    ...
    nummerierung = Synonym(block)
    RWpar = Series(Series(Drop(Text("\\RWpar")), dwsp__), nummerierung, heading, ZeroOrMore(sequence))
    ...
    RWparagraphs = OneOrMore(Series(Option(_WSPC), RWpar))
    aufzb = Series(Series(Drop(Text("\\begin{aufzb}")), dwsp__), Option(config), Option(_WSPC), ZeroOrMore(Alternative(item, Series(command, dwsp__))), Series(Drop(Text("\\end{aufzb}")), dwsp__), mandatory=4)
    aufza = Series(Series(Drop(Text("\\begin{aufza}")), dwsp__), Option(config), Option(_WSPC), ZeroOrMore(Alternative(item, Series(command, dwsp__))), Series(Drop(Text("\\end{aufza}")), dwsp__), mandatory=4)
    inparaenum = Series(Series(Drop(Text("\\begin{inparaenum}")), dwsp__), Option(config), Option(_WSPC), ZeroOrMore(Alternative(item, Series(command, dwsp__))), Series(Drop(Text("\\end{inparaenum}")), dwsp__), mandatory=4)
    compactenum = Series(Series(Drop(Text("\\begin{compactenum}")), dwsp__), Option(config), Option(_WSPC), ZeroOrMore(Alternative(item, Series(command, dwsp__))), Series(Drop(Text("\\end{compactenum}")), dwsp__), mandatory=4)
    ...
    an_enumeration = Alternative(itemize, enumerate, description, compactenum, aufza, aufzb, inparaenum)
    ...
    known_environment = Alternative(an_enumeration, figure, tabular, quotation, verbatim, math_block)
    _has_block_start = Drop(Alternative(Drop(Text("\\begin{")), Drop(Text("\\["))))
    preamble = OneOrMore(Series(Option(_WSPC), command))
    RWch = Series(Drop(Text("\\RWch")), Option(hide_from_toc), Option(config), heading, ZeroOrMore(Alternative(RWparagraphs, sequence)))
    ...
    RWchapters = OneOrMore(Series(Option(_WSPC), RWch))
    ...
    RWteil = Series(Series(Drop(Text("\\RWteil")), dwsp__), Option(config), nummerierung, heading, block, block, ZeroOrMore(Alternative(RWchapters, RWparagraphs, sequence)))
    RWteile = OneOrMore(Series(Option(_WSPC), Alternative(RWteil, RWch)))
    document = Series(Option(_WSPC), Series(Drop(Text("\\begin{document}")), dwsp__), frontpages, Alternative(Chapters, Sections, RWteile), Option(Bibliography), Option(Index), Option(_WSPC), Series(Drop(Text("\\end{document}")), dwsp__), Option(_WSPC), EOF, mandatory=2)
