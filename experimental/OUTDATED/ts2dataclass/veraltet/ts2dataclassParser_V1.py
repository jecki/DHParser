...
from DHParser import start_logging, suspend_logging, resume_logging, is_filename, load_if_file, \
    ...
    pick_from_trail, json_dumps, RootNode
...
def ts2dataclassTransformer() -> TransformerCallable:
    ...
    return partial(traverse, transformation_table=ts2dataclass_AST_transformation_table.copy())
...
class ts2dataclassCompiler(Compiler):
    ...
    def on_document(self, node) -> str:
        return '\n\n'.join(self.compile(child) for child in node.children if child.name != 'declaration')
