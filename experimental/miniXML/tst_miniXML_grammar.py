...
"""tst_miniXML_grammar.py - runs the unit tests for the miniXML-grammar
"""
...
def recompile_grammar(grammar_src, force):
    ...
    if not dsl.recompile_grammar(grammar_src, force=force,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('miniXML_ebnf_ERRORS.txt', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    if arg.endswith('.ebnf'):
        ...
        recompile_grammar(os.path.join(scriptpath, 'miniXML.ebnf'),
                          force=False)
        sys.path.append('.')
        from miniXMLParser import get_grammar, get_transformer
