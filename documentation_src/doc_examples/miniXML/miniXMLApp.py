...
"""miniXMLApp.py - a simple GUI for the compilation of miniXML-files"""
...
import miniXMLParser


class miniXMLApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('miniXML App')
        ...
        if 'html' in miniXMLParser.targets or 'HTML' in miniXMLParser.targets:
            self.target = 'html'
        elif len(miniXMLParser.targets) == 1:
            self.target = list(miniXMLParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = miniXMLParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not miniXMLParser.main(called_from_app=True):
        app = miniXMLApp()
        app.mainloop()
