...
"""simplifiedJSONServer.py - starts a server (if not already running) for the
                    compilation of simplifiedJSON
"""
...
class simplifiedJSONCPUBoundTasks:
...
class simplifiedJSONBlockingTasks:
    def __init__(self, lsp_data: dict):
        from DHParser.lsp import gen_lsp_table
        self.lsp_data = lsp_data
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
...
class simplifiedJSONLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "simplifiedJSON-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = simplifiedJSONCPUBoundTasks(self.lsp_data)
        self.blocking = simplifiedJSONBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from simplifiedJSONParser import batch_process
        args = argstr.split(' ')
    ...
    #     from simplifiedJSONParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new simplifiedJSONServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('simplifiedJSON_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    simplifiedJSON_lsp = simplifiedJSONLanguageServerProtocol()
    lsp_table = simplifiedJSON_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', simplifiedJSON_lsp.simply_compile)
    simplifiedJSON_server = Server(rpc_functions=lsp_table,
                        cpu_bound=simplifiedJSON_lsp.cpu_bound.lsp_table.keys(),
                        blocking=simplifiedJSON_lsp.blocking.lsp_table.keys(),
                        connection_callback=simplifiedJSON_lsp.connect,
                        server_name="simplifiedJSONServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        simplifiedJSON_server.echo_log = True if port >= 0 and host else False
        msg = simplifiedJSON_server.start_logging(log_path.strip('" \''))
        if simplifiedJSON_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        simplifiedJSON_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            simplifiedJSON_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing simplifiedJSON-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python simplifiedJSONServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python simplifiedJSONServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python simplifiedJSONServer.py --stream\n'
             + '    python simplifiedJSONServer.py --stopserver\n'
             + '    python simplifiedJSONServer.py --status\n'
             + '    python simplifiedJSONServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python simplifiedJSONServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
