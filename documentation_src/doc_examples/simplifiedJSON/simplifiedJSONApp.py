...
"""simplifiedJSONApp.py - a simple GUI for the compilation of simplifiedJSON-files"""
...
import simplifiedJSONParser


class simplifiedJSONApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('simplifiedJSON App')
        ...
        if 'html' in simplifiedJSONParser.targets or 'HTML' in simplifiedJSONParser.targets:
            self.target = 'html'
        elif len(simplifiedJSONParser.targets) == 1:
            self.target = list(simplifiedJSONParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = simplifiedJSONParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not simplifiedJSONParser.main(called_from_app=True):
        app = simplifiedJSONApp()
        app.mainloop()
