...
.. note:: Other than in the command-line examples presented here one would usually
    not provide the custom code as string to the create_parser()-function. Rather,
    it suffices that the code (e.g. class SaveSource) is reachable from the Grammar-class.
    For the autogenerated Grammar classes of a DHParser-project it must be placed (or imported)
    in the "...Parser.py"-file somewhere before the automatically (re-)generated Parser-section!
