...
class TestCustomizedResumeParsing_with_autogenerated_Parsers:
    ...
    def test_interaction_resume_capture(self):
        ...
        # for entry in parser.history__:  print(entry)
        # 60 in case of full left recursion, 36 in case of simple left recursion
        assert len(parser.history__) in (60, 58, 36),  len(parser.history__)  # don't repeat history because of recursion!
        # possible differences in length above are due to optimizations turned on or off!
...
class TestRuleOrder:
    ...
    def test_recursive_root(self):
        ...
        # set_config_value('compiled_EBNF_log', 'auto_reorder_test.log')
        parser = create_parser(reverse_order)
        assert parser.B.__class__.__name__ in \
               ("ForwardBase", "Forward", "SimpleForwardRecursive", "SimpleForwardIterative")
