...
class TestDHParserCommandLineTool:
    ...
    def test_dhparser_project_test(self):
        ...
        system(self.python + os.path.join(name, 'tst_%s_grammar.py --singlethread ' % name)
               + self.nulldevice)
        parser_file_name = os.path.join(name, '%sParser.py ' % name)
        assert os.path.exists(parser_file_name. rstrip())
        system(self.python + parser_file_name
    ...
    def test_rerun_after_recompiling(self):
        ...
        parser_call = self.python + ' ' + os.path.join(name, 'ArithmeticParser.py') \
                      + ' ' + ' "3 + 4 * 3" '
        output = subprocess.check_output(parser_call, shell=True)
