...
It will return the concrete-syntax-tree of the document. Various flavors of
EBNF or PEG (Parsing-Expression-Grammar) notations are supported.
...
by a comment-block with a section-name:
...
EBNF_AST_transformation_table = {
    ...
    "oneormore, repetition, option":
        [remove_tokens('?', '*', '+'), apply_if(reduce_single_child, has_child('group')),
         remove_brackets,  # remove_tokens('?', '*', '+'),
...
class EBNFDirectives:
    ...
    :ivar error:  mapping of symbols to tuples of match conditions and
            ...
            be displayed. An empty string '' as a search condition
    ...
    :ivar skip:  mapping of symbols to a list of search expressions. A
            search expression can be either a string or a regular
    ...
    :ivar resume:  mapping of symbols to a list of search expressions. A
            search expression can be either a string or a regular
            expression. The closest match is the point of reentry
            after a parsing error has occurred. Other than with
            the skip field, resuming takes place after
    ...
    def __init__(self):
        ...
        self.tokens: MutableSet[str]       = set()
        self.filter: Dict[str, str]        = dict()
        self.error: Dict[str, List[Tuple[ReprType, ReprType]]] = dict()
        self.skip: Dict[str, List[ReprType]] = dict()
        self.resume: Dict[str, List[ReprType]] = dict()
        self.disposable: str               = get_config_value('default_disposable_regexp')  # aka "hide"
        self.drop: MutableSet[str]         = set()
        self.reduction: int                = FLATTEN
        self.optimizations: FrozenSet[str] = get_config_value('optimizations')
        self.flavor: str                   = get_config_value('syntax_variant')
        self._super_ws: Optional[str]      = None
...
class EBNFCompiler(Compiler):
    ...
    :ivar referred_symbols_cache: A dictionary that caches the results of
            the method ``referred_symbols()``. ``referred_symbols()`` maps a
            symbol to the set of symbols that are directly or indirectly
            referred to in the definition of the symbol.

    :ivar directly_referred_cache: A dictionary that caches the results
            of the method :py:meth:`directly_referred_symbols`, which yields
    ...
    :ivar referred_otherwise: A set of symbols which are directly
           ...
           not matter whether these symbols are reachable (i.e. directly
           or indirectly referred to) from the root-symbol.
    ...
    :ivar variables:  A set of symbols that are used with the
    ...
    :ivar macro_stack:  A stack (i.e. list) of macro names that is needed
            to ensure that macro calls are not recursively nested.
            ...
            if node.children[1].name == "symbol":
                ...
                if key == 'whitespace' and value in WHITESPACE_TYPES:
                    ...
                    self.tree.new_error(
                        ...
                        f'Expected: {VALID_DIRECTIVES[key]}')
            else:
