...
class Parser:
    ...
    :ivar \_parse_proxy: Usually, just a reference to ``self._parse``, but can
                be overwritten to run the call to the ``_parse``-method
...
class LeafParser(Parser):
    ...
    def __call__(self: Parser, location: cython.int) -> ParsingResult:
        ...
        try:
            ...
            # if the location has already been visited by, return the saved result
            visited = self.visited  # using local variable for better performance
            if location in visited:
            ...
            for entry, parser in cdict.items():
                if isinstance(parser, Parser) and entry not in RESERVED_PARSER_NAMES:
                    ...
                    if isinstance(parser, Forward):
                        ...
                        delegate = cdict[fw.parser_name] if fw.parser_name else fw.parser
                        if not delegate.pname:
                            fw.parser_name = anonymous + entry
    ...
    def _add_parser__(self, parser: Parser) -> None:
        ...
        if parser not in self.all_parsers__:
            if parser.pname:
                ...
                fw = cast(Forward, parser)
                if fw.parser_name:
                    parser.set(getattr(self, fw.parser_name))
                else:
                    fw.parser_name = fw.parser.pname
...
forward-declaration of parsers. Please use the new inline-syntax (e.g.
`Forward("expression") ... expression = ...` for, formerly and now deprecated 
`expression = Forward() ... expression.set(...)`) instead! 

If your parser has been autogenerated from an EBFN-grammar, it suffices to 
recreate the parser with `dhparser GRAMMAR.ebnf` to update the usage pattern
of Forward-references.

The reson for this change is that the old pattern does not work with 
interwoven left-recursion, which requires that each Forward-object is
referenced only once in the grammar.

Full example for the correct usage pattern:
...
class Forward(LateBindingUnary):
    r"""
