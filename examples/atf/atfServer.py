...
"""atfServer.py - starts a server (if not already running) for the
                    compilation of atf
...
class atfLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "atf-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
...
def run_server(host, port, log_path=None):
    ...
    while ports:
        ...
        try:
            ...
            atf_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing atf-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python atfServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python atfServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python atfServer.py --stream\n'
             + '    python atfServer.py --stopserver\n'
             + '    python atfServer.py --status\n'
             + '    python atfServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python atfServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
