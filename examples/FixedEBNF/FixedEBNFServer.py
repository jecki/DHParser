...
"""FixedEBNFServer.py - starts a server (if not already running) for the
                    compilation of FixedEBNF
...
class FixedEBNFLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "FixedEBNF-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = FixedEBNFCPUBoundTasks(self.lsp_data)
        self.blocking = FixedEBNFBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from FixedEBNFParser import batch_process
        args = argstr.split(' ')
    ...
    #     from FixedEBNFParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new FixedEBNFServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    # from tst_FixedEBNF_grammar import recompile_grammar
    # recompile_grammar(os.path.join(scriptpath, 'FixedEBNF.ebnf'), force=False)
    ...
    FixedEBNF_lsp = FixedEBNFLanguageServerProtocol()
    lsp_table = FixedEBNF_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', FixedEBNF_lsp.simply_compile)
    FixedEBNF_server = Server(rpc_functions=lsp_table,
                        cpu_bound=FixedEBNF_lsp.cpu_bound.lsp_table.keys(),
                        blocking=FixedEBNF_lsp.blocking.lsp_table.keys(),
                        connection_callback=FixedEBNF_lsp.connect,
                        server_name="FixedEBNFServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        FixedEBNF_server.echo_log = True if port >= 0 and host else False
        msg = FixedEBNF_server.start_logging(log_path.strip('" \''))
        if FixedEBNF_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        FixedEBNF_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            FixedEBNF_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing FixedEBNF-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python FixedEBNFServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python FixedEBNFServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python FixedEBNFServer.py --stream\n'
             + '    python FixedEBNFServer.py --stopserver\n'
             + '    python FixedEBNFServer.py --status\n'
             + '    python FixedEBNFServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python FixedEBNFServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
