...
"""FixedEBNFApp.py - a simple GUI for the compilation of FixedEBNF-files"""
...
import FixedEBNFParser


class FixedEBNFApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('FixedEBNF App')
        ...
        if 'html' in FixedEBNFParser.targets or 'HTML' in FixedEBNFParser.targets:
            self.target = 'html'
        elif len(FixedEBNFParser.targets) == 1:
            self.target = list(FixedEBNFParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = FixedEBNFParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not FixedEBNFParser.main(called_from_app=True):
        app = FixedEBNFApp()
        app.mainloop()
