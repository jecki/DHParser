...
class FixedEBNFGrammar(Grammar):
    ...
    disposable__ = re.compile('(?:EOF$|countable$|is_mdef$|pure_elem$|no_range$|ANY_SUFFIX$|MOD_SYM$|MOD_SEP$|FOLLOW_UP$|component$)')
    ...
    placeholder = Series(SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, SmartRE(f'(?!\\()', '!`(`'), dwsp__)
    ...
    plaintext = SmartRE(f'((?:`(?:(?<!\\\\)(?:\\\\\\\\)*\\\\`|[^`])*?`)|(?:´(?:(?<!\\\\)(?:\\\\\\\\)*\\\\´|[^´])*?´))(?:{WSP_RE__})', '/`(?:(?<!\\\\)(?:\\\\\\\\)*\\\\`|[^`])*?`/|/´(?:(?<!\\\\)(?:\\\\\\\\)*\\\\´|[^´])*?´/ ~')
    literal = SmartRE(f'((?:"(?:(?<!\\\\)(?:\\\\\\\\)*\\\\"|[^"])*?")|(?:\'(?:(?<!\\\\)(?:\\\\\\\\)*\\\\\'|[^\'])*?\')|(?:’(?:(?<!\\\\)(?:\\\\\\\\)*\\\\’|[^’])*?’))(?:{WSP_RE__})', '/"(?:(?<!\\\\)(?:\\\\\\\\)*\\\\"|[^"])*?"/|/\'(?:(?<!\\\\)(?:\\\\\\\\)*\\\\\'|[^\'])*?\'/|/’(?:(?<!\\\\)(?:\\\\\\\\)*\\\\’|[^’])*?’/ ~')
    ...
    retrieveop = SmartRE(f'(?P<:Text>::|:\\?|:)(?:{WSP_RE__})', '"::"|":?"|":" ~')
    flowmarker = SmartRE(f'(?P<:Text>!|\\&|<\\-!|<\\-\\&)(?:{WSP_RE__})', '"!"|"&"|"<-!"|"<-&" ~')
    ANY_SUFFIX = RegExp('[?*+]')
    ...
    MOD_SEP = RegExp(' *: *')
    hide = SmartRE(f'(?P<:Text>HIDE|Hide|hide|DISPOSE|Dispose|dispose)(?:{WSP_RE__})', '"HIDE"|"Hide"|"hide"|"DISPOSE"|"Dispose"|"dispose" ~')
    drop = SmartRE(f'(?P<:Text>DROP|Drop|drop|SKIP|Skip|skip)(?:{WSP_RE__})', '"DROP"|"Drop"|"drop"|"SKIP"|"Skip"|"skip" ~')
    ...
    difference = Series(term, Option(Series(SmartRE(f'(?!\\->)', '!`->`'), SmartRE(fr"(?P<:Text>\-)(?:{WSP_RE__})", "\"-\""), part, mandatory=2)))
    ...
    component = Alternative(regexp, literals, procedure, Series(symbol, NegativeLookahead(is_def)), Series(SmartRE(f'(?=\\$)', '&`$`'), NegativeLookahead(is_mdef), placeholder, NegativeLookahead(is_def), mandatory=2), Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), expression, SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\"")), Series(RAISE_EXPR_WO_BRACKETS, expression))
