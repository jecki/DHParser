...
class FixedEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "e51efcbe1f47f1bdabcc4c23b17aecbe"
    disposable__ = re.compile('(?:$.)|(?:ANY_SUFFIX$|FOLLOW_UP$|pure_elem$|MOD_SEP$|EOF$|countable$|is_mdef$|component$|no_range$|MOD_SYM$)')
    ...
    placeholder = Series(SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, NegativeLookahead(Text("(")), dwsp__)
    multiplier = Series(RegExp('[1-9]\\d*'), dwsp__)
    whitespace = Series(RegExp('~'), dwsp__)
    any_char = SmartRE(fr"(?P<:Text>\.)(?:{WSP_RE__})", "\".\"")
    ...
    char_range = Series(Text("["), Option(Text("^")), OneOrMore(restricted_range_desc), SmartRE(fr"(?P<:Text>\])(?:{WSP_RE__})", "\"]\""))
    ...
    parser = Series(SmartRE(fr"(?P<:Text>@)(?:{WSP_RE__})", "\"@\""), name, SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), Option(argument), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""), mandatory=3)
    no_range = Drop(Alternative(NegativeLookahead(multiplier), Series(Lookahead(multiplier), TIMES)))
    macro = Series(SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), no_range, expression, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), no_range, expression)), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""))
    ...
    option = Alternative(Series(NegativeLookahead(char_range), SmartRE(fr"(?P<:Text>\[)(?:{WSP_RE__})", "\"[\""), expression, SmartRE(fr"(?P<:Text>\])(?:{WSP_RE__})", "\"]\""), mandatory=2), Series(element, SmartRE(fr"(?P<:Text>\?)(?:{WSP_RE__})", "\"?\"")))
    repetition = Alternative(Series(SmartRE(fr"(?P<:Text>\{{)(?:{WSP_RE__})", "\"{\""), no_range, expression, SmartRE(fr"(?P<:Text>\}})(?:{WSP_RE__})", "\"}\""), mandatory=2), Series(element, SmartRE(fr"(?P<:Text>\*)(?:{WSP_RE__})", "\"*\""), no_range))
    oneormore = Alternative(Series(SmartRE(fr"(?P<:Text>\{{)(?:{WSP_RE__})", "\"{\""), no_range, expression, SmartRE(fr"(?P<:Text>\}}\+)(?:{WSP_RE__})", "\"}+\"")), Series(element, SmartRE(fr"(?P<:Text>\+)(?:{WSP_RE__})", "\"+\"")))
    group = Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), no_range, expression, SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""), mandatory=2)
    retrieveop = Alternative(SmartRE(fr"(?P<:Text>::)(?:{WSP_RE__})", "\"::\""), SmartRE(fr"(?P<:Text>:\?)(?:{WSP_RE__})", "\":?\""), SmartRE(fr"(?P<:Text>:)(?:{WSP_RE__})", "\":\""))
    flowmarker = Alternative(SmartRE(fr"(?P<:Text>!)(?:{WSP_RE__})", "\"!\""), SmartRE(fr"(?P<:Text>\&)(?:{WSP_RE__})", "\"&\""), SmartRE(fr"(?P<:Text><\-!)(?:{WSP_RE__})", "\"<-!\""), SmartRE(fr"(?P<:Text><\-\&)(?:{WSP_RE__})", "\"<-&\""))
    ANY_SUFFIX = RegExp('[?*+]')
    is_mdef = Series(SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, Option(Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), placeholder, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), placeholder)), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""))), dwsp__, DEF)
    pure_elem = Series(element, NegativeLookahead(ANY_SUFFIX), mandatory=1)
    MOD_SEP = RegExp(' *: *')
    hide = Alternative(SmartRE(fr"(?P<:Text>HIDE)(?:{WSP_RE__})", "\"HIDE\""), SmartRE(fr"(?P<:Text>Hide)(?:{WSP_RE__})", "\"Hide\""), SmartRE(fr"(?P<:Text>hide)(?:{WSP_RE__})", "\"hide\""), SmartRE(fr"(?P<:Text>DISPOSE)(?:{WSP_RE__})", "\"DISPOSE\""), SmartRE(fr"(?P<:Text>Dispose)(?:{WSP_RE__})", "\"Dispose\""), SmartRE(fr"(?P<:Text>dispose)(?:{WSP_RE__})", "\"dispose\""))
    drop = Alternative(SmartRE(fr"(?P<:Text>DROP)(?:{WSP_RE__})", "\"DROP\""), SmartRE(fr"(?P<:Text>Drop)(?:{WSP_RE__})", "\"Drop\""), SmartRE(fr"(?P<:Text>drop)(?:{WSP_RE__})", "\"drop\""), SmartRE(fr"(?P<:Text>SKIP)(?:{WSP_RE__})", "\"SKIP\""), SmartRE(fr"(?P<:Text>Skip)(?:{WSP_RE__})", "\"Skip\""), SmartRE(fr"(?P<:Text>skip)(?:{WSP_RE__})", "\"skip\""))
    ...
    difference = Series(term, Option(Series(NegativeLookahead(Text("->")), SmartRE(fr"(?P<:Text>\-)(?:{WSP_RE__})", "\"-\""), part, mandatory=2)))
    lookaround = Series(flowmarker, part, mandatory=1)
    interleave = Series(difference, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>°)(?:{WSP_RE__})", "\"°\""), Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), difference)))
    sequence = Series(Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround), ZeroOrMore(Series(AND, dwsp__, Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround))))
    ...
    procedure = Series(SYM_REGEX, SmartRE(fr"(?P<:Text>\(\))(?:{WSP_RE__})", "\"()\""))
    literals = OneOrMore(literal)
    component = Alternative(regexp, literals, procedure, Series(symbol, NegativeLookahead(is_def)), Series(Lookahead(Text("$")), NegativeLookahead(is_mdef), placeholder, NegativeLookahead(is_def), mandatory=2), Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), expression, SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\"")), Series(RAISE_EXPR_WO_BRACKETS, expression))
    directive = Series(SmartRE(fr"(?P<:Text>@)(?:{WSP_RE__})", "\"@\""), symbol, SmartRE(fr"(?P<:Text>=)(?:{WSP_RE__})", "\"=\""), component, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), component)), Lookahead(FOLLOW_UP), mandatory=1)
    macrodef = Series(Option(modifier), SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, dwsp__, Option(Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), placeholder, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), placeholder)), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""), mandatory=1)), DEF, dwsp__, Option(Series(OR, dwsp__)), macrobody, Option(Series(MOD_SYM, dwsp__, hide)), ENDL, dwsp__, Lookahead(FOLLOW_UP))
    element.set(Alternative(Series(Option(retrieveop), symbol, NegativeLookahead(is_def)), literal, plaintext, char_ranges, Series(character, dwsp__), regexp, char_range, any_char, whitespace, group, Series(macro, NegativeLookahead(is_def)), Series(placeholder, NegativeLookahead(is_def)), parser))
