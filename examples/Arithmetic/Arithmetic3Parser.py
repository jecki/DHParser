...
read_local_config(os.path.join(scriptdir, 'Arithmetic3Config.ini'))
...
RE_COMMENT = NEVER_MATCH_PATTERN  # THIS MUST ALWAYS BE THE SAME AS Arithmetic3Grammar.COMMENT__ !!!


def Arithmetic3Tokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    Arithmetic3Tokenizer, RE_INCLUDE, RE_COMMENT)
...
class Arithmetic3Grammar(Grammar):
    r"""Parser for an Arithmetic3 document.
    ...
    """
    expression = SimpleForwardIterative()
    source_hash__ = "88725bd0d1247b104eab0fa0d12f95a2"
    ...
    term = Alternative(number, group)
    expression.set(Series(term, ZeroOrMore(Series(Alternative(Text("+"), Text("-"), Text("*"), Text(":")), term))))
    formulae = Series(wsp__, expression, ZeroOrMore(Series(wsp__, expression)), wsp__)
    root__ = formulae
    
parsing: PseudoJunction = create_parser_junction(Arithmetic3Grammar)
get_grammar = parsing.factory  # for backwards compatibility, only


try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        RE_COMMENT in (Arithmetic3Grammar.COMMENT__, NEVER_MATCH_PATTERN), \
        "Please adjust the pre-processor-variable RE_COMMENT in file Arithmetic3Parser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class Arithmetic3Grammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, Arithmetic3Grammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==Arithmetic3Grammar.COMMENT__ allows the " \
...
Arithmetic3_AST_transformation_table = {
    # AST Transformations for the Arithmetic3-grammar
    # Special rules:
...
#     Arithmetic3_AST_transformation_table, "CST", "AST", "transtable")

def Arithmetic3Transformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=Arithmetic3_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(Arithmetic3Transformer), 'AST')
...
class Arithmetic3Compiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        Arithmetic3 source file.
    """

    def __init__(self):
        super(Arithmetic3Compiler, self).__init__()
    ...
    def prepare(self, root: RootNode) -> None:
        ...
        root.stage = "Arithmetic3"
    def finalize(self, result: Any) -> Any:
        return result

    def on_formulae(self, node):
...
compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(Arithmetic3Compiler), 'Arithmetic3')
...
# # ("Arithmetic3") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "Arithmetic3", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "Arithmetic3",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('Arithmetic3_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from Arithmetic3Parser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "Arithmetic3"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " Arithmetic3 file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('Arithmetic3_serializations', serializations, allow_new_key=True)
        finalize_presets()
