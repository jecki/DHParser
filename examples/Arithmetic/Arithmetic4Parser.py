...
read_local_config(os.path.join(scriptdir, 'Arithmetic4Config.ini'))
...
RE_COMMENT = '#.*'  # THIS MUST ALWAYS BE THE SAME AS Arithmetic4Grammar.COMMENT__ !!!


def Arithmetic4Tokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    Arithmetic4Tokenizer, RE_INCLUDE, RE_COMMENT)
...
class Arithmetic4Grammar(Grammar):
    r"""Parser for an Arithmetic4 document.
    ...
    term = SimpleForwardIterative()
    source_hash__ = "11ede0fefbeb95f74fedbf872623d08b"
...
try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        RE_COMMENT in (Arithmetic4Grammar.COMMENT__, NEVER_MATCH_PATTERN), \
        "Please adjust the pre-processor-variable RE_COMMENT in file Arithmetic4Parser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class Arithmetic4Grammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, Arithmetic4Grammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==Arithmetic4Grammar.COMMENT__ allows the " \
...
Arithmetic4_AST_transformation_table = {
    # AST Transformations for the Arithmetic4-grammar
    # Special rules:
...
#     Arithmetic4_AST_transformation_table, "CST", "AST", "transtable")

def Arithmetic4Transformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=Arithmetic4_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(Arithmetic4Transformer), 'AST')
...
class Arithmetic4Compiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        Arithmetic4 source file.
    """

    def __init__(self):
        super(Arithmetic4Compiler, self).__init__()
    ...
    # def on_var(self, node):
    #     return node



compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(Arithmetic4Compiler), 'Arithmetic4')
...
# # ("Arithmetic4") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "Arithmetic4", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "Arithmetic4",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('Arithmetic4_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from Arithmetic4Parser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "Arithmetic4"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " Arithmetic4 file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('Arithmetic4_serializations', serializations, allow_new_key=True)
        finalize_presets()
