...
read_local_config(os.path.join(scriptdir, 'Arithmetic3bConfig.ini'))
...
RE_COMMENT = NEVER_MATCH_PATTERN  # THIS MUST ALWAYS BE THE SAME AS Arithmetic3bGrammar.COMMENT__ !!!


def Arithmetic3bTokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    Arithmetic3bTokenizer, RE_INCLUDE, RE_COMMENT)
...
class Arithmetic3bGrammar(Grammar):
    r"""Parser for an Arithmetic3b document.
    ...
    source_hash__ = "c2d330860c44be44f5f29e6661dcf7ea"
...
try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        RE_COMMENT in (Arithmetic3bGrammar.COMMENT__, NEVER_MATCH_PATTERN), \
        "Please adjust the pre-processor-variable RE_COMMENT in file Arithmetic3bParser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class Arithmetic3bGrammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, Arithmetic3bGrammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==Arithmetic3bGrammar.COMMENT__ allows the " \
...
Arithmetic3b_AST_transformation_table = {
    # AST Transformations for the Arithmetic3b-grammar
    # Special rules:
...
#     Arithmetic3b_AST_transformation_table, "CST", "AST", "transtable")

def Arithmetic3bTransformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=Arithmetic3b_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(Arithmetic3bTransformer), 'AST')
...
class Arithmetic3bCompiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        Arithmetic3b source file.
    """

    def __init__(self):
        super(Arithmetic3bCompiler, self).__init__()
...
compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(Arithmetic3bCompiler), 'Arithmetic3b')
...
# # ("Arithmetic3b") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "Arithmetic3b", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "Arithmetic3b",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('Arithmetic3b_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from Arithmetic3bParser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "Arithmetic3b"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " Arithmetic3b file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('Arithmetic3b_serializations', serializations, allow_new_key=True)
        finalize_presets()
