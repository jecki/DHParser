...
read_local_config(os.path.join(scriptdir, 'Arithmetic6Config.ini'))
...
RE_COMMENT = NEVER_MATCH_PATTERN  # THIS MUST ALWAYS BE THE SAME AS Arithmetic6Grammar.COMMENT__ !!!


def Arithmetic6Tokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    Arithmetic6Tokenizer, RE_INCLUDE, RE_COMMENT)
...
class Arithmetic6Grammar(Grammar):
    r"""Parser for an Arithmetic6 document.
    ...
    source_hash__ = "689a72c522814bb44fb4be4c6df8c927"
...
try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        ...
        "Please adjust the pre-processor-variable RE_COMMENT in file Arithmetic6Parser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class Arithmetic6Grammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, Arithmetic6Grammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==Arithmetic6Grammar.COMMENT__ allows the " \
...
Arithmetic6_AST_transformation_table = {
    # AST Transformations for the Arithmetic6-grammar
    # Special rules:
...
#     Arithmetic6_AST_transformation_table, "CST", "AST", "transtable")

def Arithmetic6Transformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=Arithmetic6_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(Arithmetic6Transformer), 'AST')
...
class Arithmetic6Compiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        Arithmetic6 source file.
    """

    def __init__(self):
        super(Arithmetic6Compiler, self).__init__()
...
compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(Arithmetic6Compiler), 'Arithmetic6')
...
# # ("Arithmetic6") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "Arithmetic6", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "Arithmetic6",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('Arithmetic6_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from Arithmetic6Parser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "Arithmetic6"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " Arithmetic6 file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('Arithmetic6_serializations', serializations, allow_new_key=True)
        finalize_presets()
