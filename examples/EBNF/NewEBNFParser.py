...
class NewEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "9c28121f71a93163d54424e7265fd240"
    ...
    interleave = Series(term, ZeroOrMore(Series(Series(Token("°"), dwsp__), term)))
    sequence = OneOrMore(Series(Option(Series(Token("§"), dwsp__)), Alternative(interleave, Series(flowmarker, Alternative(oneormore, pure_elem)))))
...
def get_compiler() -> NewEBNFCompiler:
