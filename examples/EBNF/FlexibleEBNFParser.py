...
class FlexibleEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "eed061f3956cc5d27342a31c75234f75"
    ...
    EOF = Drop(Drop(Series(Drop(NegativeLookahead(RegExp('.'))), Drop(Option(Drop(Pop(DEF, match_func=optional_last_value)))), Drop(Option(Drop(Pop(OR, match_func=optional_last_value)))), Drop(Option(Drop(Pop(AND, match_func=optional_last_value)))), Drop(Option(Drop(Pop(ENDL, match_func=optional_last_value)))), Drop(Option(Drop(Pop(RNG_DELIM, match_func=optional_last_value)))), Drop(Option(Drop(Pop(BRACE_SIGN, match_func=optional_last_value)))))))
    ...
    multiplier = RegExp('\\d+')
    RANGE = Series(RNG_BRACE, multiplier, Retrieve(RNG_DELIM), multiplier, Pop(RNG_BRACE, match_func=matching_bracket))
    counted = Alternative(Series(element, RANGE), Series(element, Series(Token('*'), dwsp__), multiplier), Series(multiplier, Series(Token('*'), dwsp__), element, mandatory=1))
