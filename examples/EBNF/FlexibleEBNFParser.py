...
class FlexibleEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "5412df660014950c1fbc2ea38cd3a668"
    ...
    literal_heuristics = Alternative(RegExp('"(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^"]*)*"'), RegExp("'(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^']*)*'"), RegExp('`(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^`]*)*`'), RegExp('´(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^´]*)*´'), RegExp('/(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^/]*)*/'))
    CH_LEADIN.set(Capture(Alternative(Token("0x"), Token("#x"))))
    RE_LEADOUT.set(Capture(Token("/")))
    RE_LEADIN.set(Capture(Alternative(Series(Token("/"), Lookahead(regex_heuristics)), Token("^/"))))
    ...
    OR.set(Capture(Alternative(Token("|"), Series(Token("/"), NegativeLookahead(regex_heuristics)))))
    ...
    char_range = Series(Token("["), NegativeLookahead(literal_heuristics), Option(Token("^")), Alternative(character, free_char), ZeroOrMore(Alternative(Series(Option(Token("-")), character), free_char)), Token("]"))
