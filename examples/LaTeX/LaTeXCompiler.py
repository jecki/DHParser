...
class LaTeXGrammar(Grammar):
    ...
    source_hash__ = "3f306dbfd09858a6fbe7110402e046eb"
    ...
    structural = Alternative(Series(DropToken("subsection"), wsp__), Series(DropToken("section"), wsp__), Series(DropToken("chapter"), wsp__), Series(DropToken("subsubsection"), wsp__), Series(DropToken("paragraph"), wsp__), Series(DropToken("subparagraph"), wsp__), Series(DropToken("item"), wsp__))
    blockcmd = Series(BACKSLASH, Alternative(Series(Alternative(Series(DropToken("begin{"), wsp__), Series(DropToken("end{"), wsp__)), Alternative(Series(DropToken("enumerate"), wsp__), Series(DropToken("itemize"), wsp__), Series(DropToken("figure"), wsp__), Series(DropToken("quote"), wsp__), Series(DropToken("quotation"), wsp__), Series(DropToken("tabular"), wsp__)), Series(DropToken("}"), wsp__)), structural, begin_generic_block, end_generic_block))
    no_command = Alternative(Series(DropToken("\\begin{"), wsp__), Series(DropToken("\\end"), wsp__), Series(BACKSLASH, structural))
    text = Series(TEXTCHUNK, ZeroOrMore(Series(S, TEXTCHUNK)))
    block = Series(RegExp('{'), wsp__, ZeroOrMore(Series(NegativeLookahead(blockcmd), text_element, S)), RegExp('}'), mandatory=3)
    cfg_text = ZeroOrMore(Alternative(Series(wsp__, text), CMDNAME, SPECIAL))
    config = Series(Series(DropToken("["), wsp__), cfg_text, Series(DropToken("]"), wsp__), mandatory=2)
    pdfinfo = Series(Series(DropToken("\\pdfinfo"), wsp__), block)
    documentclass = Series(Series(DropToken("\\documentclass"), wsp__), Option(config), block)
    cline = Series(Series(DropToken("\\cline{"), wsp__), INTEGER, Series(DropToken("-"), wsp__), INTEGER, Series(DropToken("}"), wsp__))
    hline = Series(Token("\\hline"), wsp__)
    multicolumn = Series(Series(DropToken("\\multicolumn"), wsp__), Series(DropToken("{"), wsp__), INTEGER, Series(DropToken("}"), wsp__), tabular_config, block_of_paragraphs)
    caption = Series(Series(DropToken("\\caption"), wsp__), block)
    includegraphics = Series(Series(DropToken("\\includegraphics"), wsp__), Option(config), block)
    footnote = Series(Series(DropToken("\\footnote"), wsp__), block_of_paragraphs)
    citep = Series(Alternative(Series(DropToken("\\citep"), wsp__), Series(DropToken("\\cite"), wsp__)), Option(config), block)
    citet = Series(Series(DropToken("\\citet"), wsp__), Option(config), block)
    generic_command = Series(NegativeLookahead(no_command), CMDNAME, Option(Series(Option(Series(wsp__, config)), wsp__, block)))
    ...
    generic_inline_env = Series(begin_inline_env, wsp__, paragraph, end_inline_env, mandatory=3)
    ...
    paragraph.set(OneOrMore(Series(NegativeLookahead(blockcmd), text_element, S)))
    sequence = Series(Option(WSPC), OneOrMore(Series(Alternative(paragraph, block_environment), Option(PARSEP))))
    block_of_paragraphs.set(Series(Series(DropToken("{"), wsp__), Option(sequence), Series(DropToken("}"), wsp__), mandatory=2))
    tabular_config.set(Series(Series(DropToken("{"), wsp__), RegExp('[lcr|]+'), wsp__, Series(DropToken("}"), wsp__), mandatory=3))
    tabular_cell = ZeroOrMore(Series(line_element, S))
    tabular_row = Series(Alternative(multicolumn, tabular_cell), ZeroOrMore(Series(Series(DropToken("&"), wsp__), Alternative(multicolumn, tabular_cell))), Series(DropToken("\\\\"), wsp__), Alternative(hline, ZeroOrMore(cline)))
    tabular = Series(Series(DropToken("\\begin{tabular}"), wsp__), tabular_config, ZeroOrMore(tabular_row), Series(DropToken("\\end{tabular}"), wsp__), mandatory=3)
    verbatim = Series(Series(DropToken("\\begin{verbatim}"), wsp__), sequence, Series(DropToken("\\end{verbatim}"), wsp__), mandatory=2)
    quotation = Alternative(Series(Series(DropToken("\\begin{quotation}"), wsp__), sequence, Series(DropToken("\\end{quotation}"), wsp__), mandatory=2), Series(Series(DropToken("\\begin{quote}"), wsp__), sequence, Series(DropToken("\\end{quote}"), wsp__), mandatory=2))
    figure = Series(Series(DropToken("\\begin{figure}"), wsp__), sequence, Series(DropToken("\\end{figure}"), wsp__), mandatory=2)
    item = Series(Series(DropToken("\\item"), wsp__), sequence)
    enumerate = Series(Series(DropToken("\\begin{enumerate}"), wsp__), Option(WSPC), ZeroOrMore(item), Series(DropToken("\\end{enumerate}"), wsp__), mandatory=3)
    itemize = Series(Series(DropToken("\\begin{itemize}"), wsp__), Option(WSPC), ZeroOrMore(item), Series(DropToken("\\end{itemize}"), wsp__), mandatory=3)
    ...
    Index = Series(Option(WSPC), Series(DropToken("\\printindex"), wsp__))
    Bibliography = Series(Option(WSPC), Series(DropToken("\\bibliography"), wsp__), heading)
    SubParagraph = Series(Series(DropToken("\\subparagraph"), wsp__), heading, Option(sequence))
    SubParagraphs = OneOrMore(Series(Option(WSPC), SubParagraph))
    Paragraph = Series(Series(DropToken("\\paragraph"), wsp__), heading, ZeroOrMore(Alternative(sequence, SubParagraphs)))
    Paragraphs = OneOrMore(Series(Option(WSPC), Paragraph))
    SubSubSection = Series(Series(DropToken("\\subsubsection"), wsp__), heading, ZeroOrMore(Alternative(sequence, Paragraphs)))
    SubSubSections = OneOrMore(Series(Option(WSPC), SubSubSection))
    SubSection = Series(Series(DropToken("\\subsection"), wsp__), heading, ZeroOrMore(Alternative(sequence, SubSubSections)))
    SubSections = OneOrMore(Series(Option(WSPC), SubSection))
    Section = Series(Series(DropToken("\\section"), wsp__), heading, ZeroOrMore(Alternative(sequence, SubSections)))
    Sections = OneOrMore(Series(Option(WSPC), Section))
    Chapter = Series(Series(DropToken("\\chapter"), wsp__), heading, ZeroOrMore(Alternative(sequence, Sections)))
    Chapters = OneOrMore(Series(Option(WSPC), Chapter))
    frontpages = Synonym(sequence)
    document = Series(Option(WSPC), Series(DropToken("\\begin{document}"), wsp__), frontpages, Alternative(Chapters, Sections), Option(Bibliography), Option(Index), Option(WSPC), Series(DropToken("\\end{document}"), wsp__), Option(WSPC), EOF, mandatory=9)
