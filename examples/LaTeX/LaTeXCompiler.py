...
class LaTeXGrammar(Grammar):
    ...
    source_hash__ = "915e53fdfd5fda93df335d804bf564df"
    ...
    GAP = Series(RegExp('[ \\t]*(?:\\n[ \\t]*)+\\n'), dwsp__)
    WSPC = OneOrMore(Alternative(comment__, Drop(RegExp('\\s+'))))
    ...
    tabular_config.set(Series(Series(Drop(Token("{")), dwsp__), Drop(RegExp('[lcr|]+')), dwsp__, Series(Drop(Token("}")), dwsp__), mandatory=3))
    ...
    enumerate = Series(Series(Drop(Token("\\begin{enumerate}")), dwsp__), Option(WSPC), ZeroOrMore(item), Series(Drop(Token("\\end{enumerate}")), dwsp__), mandatory=3)
    itemize = Series(Series(Drop(Token("\\begin{itemize}")), dwsp__), Option(WSPC), ZeroOrMore(item), Series(Drop(Token("\\end{itemize}")), dwsp__), mandatory=3)
    ...
    Index = Series(Option(WSPC), Series(Drop(Token("\\printindex")), dwsp__))
    Bibliography = Series(Option(WSPC), Series(Drop(Token("\\bibliography")), dwsp__), heading)
    ...
    document = Series(Option(WSPC), Series(Drop(Token("\\begin{document}")), dwsp__), frontpages, Alternative(Chapters, Sections), Option(Bibliography), Option(Index), Option(WSPC), Series(Drop(Token("\\end{document}")), dwsp__), Option(WSPC), EOF, mandatory=9)
