...
# The following version would only compatible with python version 2.2 and above!
...
##    respectively.
##
##    The  class is syntactical sugar to provide similar access to
##    pay functions or lists as to Numeric or nummarray arrays.
##
##    Warning: This wrapper does not do any error checking at all!
##    """
##    
...
def RandomDistribution(n):
    ...
    r.sort()
    r.extend([1.0,0.0]) 
    return [r[i]-r[i-1] for i in range(n)]
...
class GenFitnessFunction(object):
    def __init__(self, payoff, e=0.0, N=2, samples=1.0):
        ...
        if N == 1:
            if isinstance(e, 0.0.__class__) and (e != 0.0):
                warnings.warn("Correlation e != %f is meaningless if N == 1"%e,
                              UserWarning)
            self.stdFunction = lambda p: [payoff[i,] for i in range(len(p))] 
        elif N == 2:
            if e == 0.0:
                self.stdFunction = _QuickFitness2
                self.params = (payoff,)
                self.sampleFunction = _SampledQuickFitness2
            else:
                self.stdFunction = _Fitness2
                self.params = (payoff, e)
                self.sampleFunction = _SampledFitness2
        elif N > 2:
            if e == 0.0:
                self.stdFunction = _QuickFitness
                self.params = (payoff, N)
                self.sampleFunction = _SampledQuickFitness
            else:
        ...
        else:  self.function = self.stdFunction
...
class GenDynamicsFunction(object):
    ...
    """
        ...
        if noise == 0.0:
            ...
            self.params = (self.fitness,)
        else:
            self.noise = noise
            self.function = _Replicator
            self.params = (self.fitness, self.noise)
