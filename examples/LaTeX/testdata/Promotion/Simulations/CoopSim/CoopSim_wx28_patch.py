...
"""
CoopSim 0.9.9 beta 5 - The Evolution of Cooperation
...
from PyPlotter import wxGfx, psGfx, Colors, Graph, Simplex
...
def gatherStrategies():
    strategyList = []
    classes = set([Strategies.Strategy])
...
class SetupDialog(wx.Dialog):
    def __init__(self, parent, setupTemplate, setupName = ""):
        ...
        wx.EVT_LISTBOX(self.pool, -1, self.OnPoolLB)
        ...
        wx.EVT_LISTBOX(self.selected, -1, self.OnSelectedLB)
    ...
    def OnPick(self, event):
        ...
        self.availableList = list(set(self.availableList) - \
                                  set(self.selectedList))
        self.availableList.sort()
        self.selectedList.sort()
...
class SimWindow(wx.Frame, Logging.LogNotificationInterface):
    def __init__(self):
        ...
        simMenu = [
            ...
            ("&Continue Simulation", self.OnContinueSim,
             ...
             "have already been calculated.", "continue"),
            None
        ]
        helpMenu = [
    ...
    def loadState(self, fileName):
        try:
            if fileName.endswith(".txt") or fileName.endswith(".py"):
                ...
                self.simSetup.cachedPM = None
                self.simSetup.cachedLog = None
                f.close()
                self.interpretCustomCode()
                # self.simulation.newSetup(self.simSetup, self.progressIndicator)
                self.clearUnsavedFlag()
    ...
    def updateSimplex(self, destDC = None):
        if self.currentPage == self.PAGE_SIMPLEX:
            ...
            if self.simplexPending == SIMPLEX_REDRAW or \
                self.simplexPending == SIMPLEX_RESIZE:
                pending = self.simplexPending
