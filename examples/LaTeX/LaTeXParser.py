...
from DHParser.nodetree import Node, WHITESPACE_PTYPE, TOKEN_PTYPE, RootNode, flatten_sxpr
...
class LaTeXGrammar(Grammar):
    ...
    source_hash__ = "f9aa20970fd84f72e7e78bbf46af28ce"
    ...
    CMDNAME = SmartRE(f'(\\\\@?(?:(?![\\d_])\\w)+)(?:{WSP_RE__})', '/\\\\@?(?:(?![\\d_])\\w)+/ ~')
    WARN_Komma = SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\"")
    esc_char = Text(",")
    ...
    info_assoc = Series(info_key, dwsp__, Option(Series(SmartRE(fr"(?:\()(?:{WSP_RE__})", "\"(\""), info_value, SmartRE(fr"(?:\))(?:{WSP_RE__})", "\")\""), mandatory=1)))
    _info_block = Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), ZeroOrMore(info_assoc), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=1)
    ...
    association = Series(key, dwsp__, SmartRE(fr"(?:=)(?:{WSP_RE__})", "\"=\""), value, dwsp__)
    parameters = Series(Alternative(association, flag), ZeroOrMore(Series(NegativeLookbehind(_BACKSLASH), SmartRE(fr"(?:,)(?:{WSP_RE__})", "\",\""), Alternative(association, flag))), Option(WARN_Komma))
    sequence = Series(Option(_WSPC), OneOrMore(Series(Alternative(paragraph, _block_environment), Option(Alternative(_PARSEP, S)))))
    block_of_paragraphs = Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), Option(sequence), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=2)
    special = Alternative(Drop(Text("\\-")), Series(Drop(RegExp('\\\\')), esc_char), UMLAUT, QUOTEMARK)
    _item_name = Text("item")
    _structure_name = Drop(SmartRE(f'subsection|section|chapter|subsubsection|paragraph|subparagraph', '`subsection`|`section`|`chapter`|`subsubsection`|`paragraph`|`subparagraph`'))
    _env_name = Drop(SmartRE(f'enumerate|itemize|description|figure|quote|quotation|tabular|displaymath\\*?|equation\\*?|eqnarray\\*?', '`enumerate`|`itemize`|`description`|`figure`|`quote`|`quotation`|`tabular`|`displaymath` [`*`]|`equation` [`*`]|`eqnarray` [`*`]'))
    blockcmd = Series(_BACKSLASH, Alternative(Series(SmartRE(f'(?:begin\\{{|end\\{{){WSP_RE__}', '"begin{"|"end{" ~'), _env_name, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\"")), Series(_structure_name, SmartRE(f'(?=\\{{)', '&`{`')), Drop(Text("[")), Drop(Text("]")), _item_name))
    no_command = Alternative(SmartRE(fr"(?:\\begin\{{)(?:{WSP_RE__})", "\"\\begin{\""), SmartRE(fr"(?:\\end\{{)(?:{WSP_RE__})", "\"\\end{\""), Series(_BACKSLASH, _structure_name, SmartRE(f'(?=\\{{)', '&`{`')))
    ...
    config = Series(SmartRE(fr"(?:\[)(?:{WSP_RE__})", "\"[\""), Alternative(Series(parameters, SmartRE(f'(?=(?:\\])(?:{WSP_RE__}))', '&"]"')), cfg_text), SmartRE(fr"(?:\])(?:{WSP_RE__})", "\"]\""), mandatory=1)
    item = Series(SmartRE(fr"(?:\\item)(?:{WSP_RE__})", "\"\\item\""), Option(config), sequence, mandatory=2)
    ...
    href = Series(SmartRE(fr"(?:\\href\{{)(?:{WSP_RE__})", "\"\\href{\""), urlstring, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), block)
    url = Series(SmartRE(fr"(?:\\url\{{)(?:{WSP_RE__})", "\"\\url{\""), urlstring, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""))
    ref = Series(SmartRE(f'(?:\\\\ref\\{{|\\\\pageref\\{{){WSP_RE__}', '"\\ref{"|"\\pageref{" ~'), CHARS, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""))
    label = Series(SmartRE(fr"(?:\\label\{{)(?:{WSP_RE__})", "\"\\label{\""), CHARS, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""))
    hypersetup = Series(SmartRE(fr"(?:\\hypersetup)(?:{WSP_RE__})", "\"\\hypersetup\""), param_block)
    pdfinfo = Series(SmartRE(fr"(?:\\pdfinfo)(?:{WSP_RE__})", "\"\\pdfinfo\""), _info_block)
    documentclass = Series(SmartRE(fr"(?:\\documentclass)(?:{WSP_RE__})", "\"\\documentclass\""), Option(config), block)
    column_nr = Synonym(_INTEGER)
    cline = Series(SmartRE(fr"(?:\\cline\{{)(?:{WSP_RE__})", "\"\\cline{\""), column_nr, SmartRE(fr"(?:\-)(?:{WSP_RE__})", "\"-\""), column_nr, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""))
    hline = SmartRE(fr"(?P<:Text>\\hline)(?:{WSP_RE__})", "\"\\hline\"")
    multicolumn = Series(SmartRE(fr"(?:\\multicolumn)(?:{WSP_RE__})", "\"\\multicolumn\""), SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), column_nr, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), tabular_config, block_of_paragraphs)
    caption = Series(SmartRE(fr"(?:\\caption)(?:{WSP_RE__})", "\"\\caption\""), block)
    includegraphics = Series(SmartRE(fr"(?:\\includegraphics)(?:{WSP_RE__})", "\"\\includegraphics\""), Option(config), block)
    footnote = Series(SmartRE(fr"(?:\\footnote)(?:{WSP_RE__})", "\"\\footnote\""), block_of_paragraphs)
    citep = Series(SmartRE(f'(?:\\\\citep|\\\\cite){WSP_RE__}', '"\\citep"|"\\cite" ~'), Option(config), block)
    citet = Series(SmartRE(fr"(?:\\citet)(?:{WSP_RE__})", "\"\\citet\""), Option(config), block)
    ...
    assignment = Series(NegativeLookahead(no_command), CMDNAME, SmartRE(fr"(?:=)(?:{WSP_RE__})", "\"=\""), Alternative(Series(number, Option(UNIT)), block, CHARS))
    ...
    SubParagraph = Series(SmartRE(fr"(?:\\subparagraph)(?:{WSP_RE__})", "\"\\subparagraph\""), heading, Option(sequence))
    hide_from_toc = SmartRE(fr"(?P<:Text>\*)(?:{WSP_RE__})", "\"*\"")
    SubParagraphs = OneOrMore(Series(Option(_WSPC), SubParagraph))
    Paragraph = Series(SmartRE(fr"(?:\\paragraph)(?:{WSP_RE__})", "\"\\paragraph\""), heading, ZeroOrMore(Alternative(sequence, SubParagraphs)))
    ...
    rb_down = Series(SmartRE(fr"(?:\[)(?:{WSP_RE__})", "\"[\""), number, UNIT, dwsp__, SmartRE(fr"(?:\])(?:{WSP_RE__})", "\"]\""))
    rb_up = Series(SmartRE(fr"(?:\[)(?:{WSP_RE__})", "\"[\""), number, UNIT, dwsp__, SmartRE(fr"(?:\])(?:{WSP_RE__})", "\"]\""))
    rb_offset = Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), number, UNIT, dwsp__, SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""))
    raisebox = Series(SmartRE(fr"(?:\\raisebox)(?:{WSP_RE__})", "\"\\raisebox\""), rb_offset, Option(rb_up), Option(rb_down), block)
    ...
    tabular_row = Series(Alternative(multicolumn, tabular_cell), ZeroOrMore(Series(SmartRE(fr"(?:\&)(?:{WSP_RE__})", "\"&\""), Alternative(multicolumn, tabular_cell))), Alternative(Series(SmartRE(fr"(?:\\\\)(?:{WSP_RE__})", "\"\\\\\""), Alternative(hline, ZeroOrMore(cline)), Option(_PARSEP)), SmartRE(f'(?=\\\\end\\{{tabular\\}})', '&`\\end{tabular}`')))
    tabular = Series(SmartRE(fr"(?:\\begin\{{tabular\}})(?:{WSP_RE__})", "\"\\begin{tabular}\""), tabular_config, ZeroOrMore(Alternative(tabular_row, _WSPC)), SmartRE(fr"(?:\\end\{{tabular\}})(?:{WSP_RE__})", "\"\\end{tabular}\""), mandatory=3)
    no_numbering = Text("*")
    _block_math = RegExp('(?:[^\\\\]*[\\\\]?(?!end\\{(?:eqnarray|equation|displaymath)\\*?\\}|\\])\\s*)*')
    eqnarray = Series(Drop(Text("\\begin{eqnarray")), Option(no_numbering), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), _block_math, Drop(Text("\\end{eqnarray")), Option(Drop(Text("*"))), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=3)
    equation = Series(Drop(Text("\\begin{equation")), Option(no_numbering), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), _block_math, Drop(Text("\\end{equation")), Option(Drop(Text("*"))), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=3)
    _dmath_short_form = Series(SmartRE(fr"(?:\\\[)(?:{WSP_RE__})", "\"\\[\""), _block_math, SmartRE(fr"(?:\\\])(?:{WSP_RE__})", "\"\\]\""), mandatory=1)
    _dmath_long_form = Series(Drop(Text("\\begin{displaymath")), Option(no_numbering), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), _block_math, Drop(Text("\\end{displaymath")), Option(Drop(Text("*"))), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=3)
    displaymath = Alternative(_dmath_long_form, _dmath_short_form)
    verbatim_text = RegExp('(?:(?!\\\\end{verbatim})[\\\\]?[^\\\\]*)*')
    verbatim = Series(SmartRE(fr"(?:\\begin\{{verbatim\}})(?:{WSP_RE__})", "\"\\begin{verbatim}\""), verbatim_text, SmartRE(fr"(?:\\end\{{verbatim\}})(?:{WSP_RE__})", "\"\\end{verbatim}\""), mandatory=2)
    quotation = Alternative(Series(SmartRE(fr"(?:\\begin\{{quotation\}})(?:{WSP_RE__})", "\"\\begin{quotation}\""), sequence, SmartRE(fr"(?:\\end\{{quotation\}})(?:{WSP_RE__})", "\"\\end{quotation}\""), mandatory=2), Series(SmartRE(fr"(?:\\begin\{{quote\}})(?:{WSP_RE__})", "\"\\begin{quote}\""), sequence, SmartRE(fr"(?:\\end\{{quote\}})(?:{WSP_RE__})", "\"\\end{quote}\""), mandatory=2))
    figure = Series(SmartRE(fr"(?:\\begin\{{figure\}})(?:{WSP_RE__})", "\"\\begin{figure}\""), sequence, SmartRE(fr"(?:\\end\{{figure\}})(?:{WSP_RE__})", "\"\\end{figure}\""), mandatory=2)
    ...
    description = Series(SmartRE(fr"(?:\\begin\{{description\}})(?:{WSP_RE__})", "\"\\begin{description}\""), _itemsequence, SmartRE(fr"(?:\\end\{{description\}})(?:{WSP_RE__})", "\"\\end{description}\""), mandatory=2)
    enumerate = Series(SmartRE(fr"(?:\\begin\{{enumerate\}})(?:{WSP_RE__})", "\"\\begin{enumerate}\""), _itemsequence, SmartRE(fr"(?:\\end\{{enumerate\}})(?:{WSP_RE__})", "\"\\end{enumerate}\""), mandatory=2)
    itemize = Series(SmartRE(fr"(?:\\begin\{{itemize\}})(?:{WSP_RE__})", "\"\\begin{itemize}\""), _itemsequence, SmartRE(fr"(?:\\end\{{itemize\}})(?:{WSP_RE__})", "\"\\end{itemize}\""), mandatory=2)
    end_generic_block = Series(end_environment, Alternative(LFF, SmartRE(f'{WSP_RE__}(?=\\}})', '~ &`}`')), mandatory=1)
    ...
    _has_block_start = Drop(SmartRE(f'\\\\begin\\{{|\\\\\\[', '`\\begin{`|`\\[`'))
    ...
    Index = Series(Option(_WSPC), SmartRE(fr"(?:\\printindex)(?:{WSP_RE__})", "\"\\printindex\""))
    Bibliography = Series(Option(_WSPC), SmartRE(fr"(?:\\bibliography)(?:{WSP_RE__})", "\"\\bibliography\""), heading)
    ...
    document = Series(Option(_WSPC), SmartRE(fr"(?:\\begin\{{document\}})(?:{WSP_RE__})", "\"\\begin{document}\""), frontpages, Alternative(Chapters, Sections), Option(Bibliography), Option(Index), Option(_WSPC), SmartRE(fr"(?:\\end\{{document\}})(?:{WSP_RE__})", "\"\\end{document}\""), Option(_WSPC), EOF, mandatory=2)
    param_block.set(Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), Option(parameters), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\"")))
    block.set(Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), _block_content, Drop(Text("}")), mandatory=2))
    ...
    tabular_config.set(Series(SmartRE(fr"(?:\{{)(?:{WSP_RE__})", "\"{\""), OneOrMore(Alternative(Series(cfg_celltype, Option(cfg_unit)), cfg_separator, Drop(RegExp(' +')))), SmartRE(fr"(?:\}})(?:{WSP_RE__})", "\"}\""), mandatory=2))
