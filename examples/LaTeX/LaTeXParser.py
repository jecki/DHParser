...
class LaTeXGrammar(Grammar):
    ...
    blockcmd = Series(_BACKSLASH, Alternative(Series(SmartRE(f'(?:begin\\{{|end\\{{){WSP_RE__}', '"begin{"|"end{" ~'), _env_name, Series(Drop(Text("}")), dwsp__)), Series(_structure_name, Lookahead(Drop(Text("{")))), Drop(Text("[")), Drop(Text("]")), _item_name))
    ...
    ref = Series(SmartRE(f'(?:\\\\ref\\{{|\\\\pageref\\{{){WSP_RE__}', '"\\ref{"|"\\pageref{" ~'), CHARS, Series(Drop(Text("}")), dwsp__))
