...
class LaTeXGrammar(Grammar):
    ...
    _structure_name = Drop(SmartRE('subsection|section|chapter|subsubsection|paragraph|subparagraph', '`subsection`|`section`|`chapter`|`subsubsection`|`paragraph`|`subparagraph`'))
    ...
    blockcmd = Series(_BACKSLASH, Alternative(Series(Series(SmartRE('begin\\{|end\\{', '"begin{"|"end{"'), dwsp__), _env_name, Series(Drop(Text("}")), dwsp__)), Series(_structure_name, Lookahead(Drop(Text("{")))), Drop(Text("[")), Drop(Text("]")), _item_name))
    ...
    ref = Series(Series(SmartRE('\\\\ref\\{|\\\\pageref\\{', '"\\ref{"|"\\pageref{"'), dwsp__), CHARS, Series(Drop(Text("}")), dwsp__))
    ...
    citep = Series(Series(SmartRE('\\\\citep|\\\\cite', '"\\citep"|"\\cite"'), dwsp__), Option(config), block)
    ...
    _has_block_start = Drop(SmartRE('\\\\begin\\{|\\\\\\[', '`\\begin{`|`\\[`'))
