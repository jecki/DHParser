...
class LaTeXGrammar(Grammar):
    ...
    source_hash__ = "01a4c3af5dce4b18906e3f90d8036cb2"
    ...
    value = Alternative(magnitude, _LETTERS, CMDNAME, Ref("param_block"), Ref("block"))
    ...
    sequence = Series(Option(_WS), OneOrMore(Series(Alternative(Ref("paragraph"), Ref("_block_environment")), Option(Alternative(_PARSEP, S)))))
    ...
    cfg_text = ZeroOrMore(Alternative(Series(dwsp__, text), CMDNAME, SPECIAL, Ref("block")))
    ...
    _block_content = Series(Option(Alternative(_PARSEP, S)), ZeroOrMore(Series(Alternative(Ref("_block_environment"), Ref("_text_element"), Ref("paragraph")), Option(Alternative(_PARSEP, S)))))
    ...
    href = Series(Series(Drop(Text("\\href{")), dwsp__), urlstring, Series(Drop(Text("}")), dwsp__), Ref("block"))
    ...
    hypersetup = Series(Series(Drop(Text("\\hypersetup")), dwsp__), Ref("param_block"))
    pdfinfo = Series(Series(Drop(Text("\\pdfinfo")), dwsp__), _info_block)
    documentclass = Series(Series(Drop(Text("\\documentclass")), dwsp__), Option(config), Ref("block"))
    ...
    multicolumn = Series(Series(Drop(Text("\\multicolumn")), dwsp__), Series(Drop(Text("{")), dwsp__), column_nr, Series(Drop(Text("}")), dwsp__), Ref("tabular_config"), block_of_paragraphs)
    caption = Series(Series(Drop(Text("\\caption")), dwsp__), Ref("block"))
    includegraphics = Series(Series(Drop(Text("\\includegraphics")), dwsp__), Option(config), Ref("block"))
    footnote = Series(Series(Drop(Text("\\footnote")), dwsp__), block_of_paragraphs)
    citep = Series(Alternative(Series(Drop(Text("\\citep")), dwsp__), Series(Drop(Text("\\cite")), dwsp__)), Option(config), Ref("block"))
    citet = Series(Series(Drop(Text("\\citet")), dwsp__), Option(config), Ref("block"))
    generic_command = Alternative(Series(NegativeLookahead(no_command), CMDNAME, ZeroOrMore(Series(dwsp__, Alternative(config, Ref("block"))))), Series(Drop(Text("{")), CMDNAME, _block_content, Drop(Text("}")), mandatory=3))
    assignment = Series(NegativeLookahead(no_command), CMDNAME, Series(Drop(Text("=")), dwsp__), Alternative(Series(number, Option(UNIT)), Ref("block"), CHARS))
    ...
    generic_inline_env = Series(_begin_inline_env, dwsp__, Ref("paragraph"), NegativeLookahead(_PARSEP), _end_inline_env, mandatory=4)
    _known_inline_env = Synonym(inline_math)
    _inline_environment = Alternative(_known_inline_env, generic_inline_env)
    _line_element = Alternative(text, _inline_environment, _command, Ref("block"))
    ...
    cfg_separator = Alternative(Drop(Text("|")), Series(Drop(Text("!")), Ref("block")))
    ...
    raisebox = Series(Series(Drop(Text("\\raisebox")), dwsp__), rb_offset, Option(rb_up), Option(rb_down), Ref("block"))
    ...
    tabular = Series(Series(Drop(Text("\\begin{tabular}")), dwsp__), Ref("tabular_config"), ZeroOrMore(Alternative(tabular_row, _WSPC)), Series(Drop(Text("\\end{tabular}")), dwsp__), mandatory=3)
    no_numbering = Text("*")
    ...
    paragraph.set(OneOrMore(Series(NegativeLookahead(blockcmd), Ref("_text_element"), Option(S))))
