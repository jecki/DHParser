...
"""tst_re_grammar.py - runs the unit tests for the re-grammar
"""
...
try:
    from DHParser import versionnumber, nil_preprocessor_factory
...
if __name__ == '__main__':
    ...
    read_local_config(os.path.join(scriptdir, 'reConfig.ini'))
    ...
    if args.scripts:
        dsl.create_scripts(os.path.join(scriptdir, 're.ebnf'))
    ...
    if arg.endswith('.ebnf'):
        ...
        recompile_grammar(os.path.join(scriptdir, 're.ebnf'),
                          force=False)
        sys.path.append('.')
        from reParser import parsing, ASTTransformation, preprocessing, \
