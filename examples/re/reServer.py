...
"""reServer.py - starts a server (if not already running) for the
                    compilation of re
"""
...
class reLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "re-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = reCPUBoundTasks(self.lsp_data)
        self.blocking = reBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from reParser import batch_process
        args = argstr.split(' ')
...
def run_server(host, port, log_path=None):
    """
    Starts a new reServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('re_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    re_lsp = reLanguageServerProtocol()
    lsp_table = re_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', re_lsp.simply_compile)
    re_server = Server(rpc_functions=lsp_table,
                        cpu_bound=re_lsp.cpu_bound.lsp_table.keys(),
                        blocking=re_lsp.blocking.lsp_table.keys(),
                        connection_callback=re_lsp.connect,
                        server_name="reServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        re_server.echo_log = True if port >= 0 and host else False
        msg = re_server.start_logging(log_path.strip('" \''))
        if re_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        re_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            re_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing re-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python reServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python reServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python reServer.py --stream\n'
             + '    python reServer.py --stopserver\n'
             + '    python reServer.py --status\n'
             + '    python reServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python reServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
