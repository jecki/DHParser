...
"""reApp.py - a simple GUI for the compilation of re-files"""
...
class reApp(tkinter.Tk):
    def __init__(self):
        ...
        if 'html' in reParser.targets or 'HTML' in reParser.targets:
            self.target = 'html'
        elif len(reParser.targets) == 1:
            self.target = list(reParser.targets)[0]
        else:
            self.target = ''
...
if __name__ == '__main__':
    ...
    if not reParser.main(called_from_app=True):
        app = reApp()
        app.mainloop()
