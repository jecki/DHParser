...
"""ts2dataclassServer.py - starts a server (if not already running) for the
                    compilation of ts2dataclass
...
class ts2dataclassCPUBoundTasks:
...
class ts2dataclassBlockingTasks:
    def __init__(self, lsp_data: dict):
        from DHParser.lsp import gen_lsp_table
        self.lsp_data = lsp_data
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
...
class ts2dataclassLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "ts2dataclass-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = ts2dataclassCPUBoundTasks(self.lsp_data)
        self.blocking = ts2dataclassBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from ts2dataclassParser import batch_process
        args = argstr.split(' ')
    ...
    #     from ts2dataclassParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new ts2dataclassServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    # from tst_ts2dataclass_grammar import recompile_grammar
    # recompile_grammar(os.path.join(scriptpath, 'ts2dataclass.ebnf'), force=False)
    ...
    ts2dataclass_lsp = ts2dataclassLanguageServerProtocol()
    lsp_table = ts2dataclass_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', ts2dataclass_lsp.simply_compile)
    ts2dataclass_server = Server(rpc_functions=lsp_table,
                        cpu_bound=ts2dataclass_lsp.cpu_bound.lsp_table.keys(),
                        blocking=ts2dataclass_lsp.blocking.lsp_table.keys(),
                        connection_callback=ts2dataclass_lsp.connect,
                        server_name="ts2dataclassServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        ts2dataclass_server.echo_log = True if port >= 0 and host else False
        msg = ts2dataclass_server.start_logging(log_path.strip('" \''))
        if ts2dataclass_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        ts2dataclass_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            ts2dataclass_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing ts2dataclass-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python ts2dataclassServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python ts2dataclassServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python ts2dataclassServer.py --stream\n'
             + '    python ts2dataclassServer.py --stopserver\n'
             + '    python ts2dataclassServer.py --status\n'
             + '    python ts2dataclassServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python ts2dataclassServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
