...
"""neuServer.py - starts a server (if not already running) for the
                    compilation of neu
...
class neuLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "neu-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = neuCPUBoundTasks(self.lsp_data)
        self.blocking = neuBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
...
def run_server(host, port, log_path=None):
    ...
    try:
        neu_server.run_server(host, port)  # returns only after server has stopped
    except OSError as e:
        print(e)
...
def print_usage_and_exit():
    print('Usages:\n'
          + '    python neuServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
          + '    python neuServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
          + '    python neuServer.py --stopserver\n'
          + '    python neuServer.py --status\n'
          + '    python neuServer.py --logging [ON|LOG_PATH|OFF]\n'
          + '    python neuServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
    sys.exit(1)
