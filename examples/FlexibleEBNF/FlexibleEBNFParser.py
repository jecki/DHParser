...
class FlexibleEBNFGrammar(Grammar):
    ...
    disposable__ = re.compile('(?:MOD_SYM$|countable$|pure_elem$|no_range$|EOF$|FOLLOW_UP$|MOD_SEP$|is_mdef$|ANY_SUFFIX$|component$)')
    ...
    RE_CORE = RegExp('(?:(?<!\\\\)\\\\(?:/)|[^/])*')
    regex_heuristics = Series(NegativeLookahead(SmartRE(f'( +`[^`]*` +/| +´[^´]*´ +/| +\'[^\']*\' +/| +"[^"]*" +/| +\\w+ +/)', '/ +`[^`]*` +\\//|/ +´[^´]*´ +\\//|/ +\'[^\']*\' +\\//|/ +"[^"]*" +\\//|/ +\\w+ +\\//')), SmartRE(f'([^/\\n*?+\\\\]*[*?+\\\\][^/\\n]*/|[^\\w]+/|[^ ])', '/[^\\/\\n*?+\\\\]*[*?+\\\\][^\\/\\n]*\\//|/[^\\w]+\\//|/[^ ]/'))
