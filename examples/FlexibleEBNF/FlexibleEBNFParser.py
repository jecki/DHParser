...
class FlexibleEBNFGrammar(Grammar):
    ...
    disposable__ = re.compile('(?:$.)|(?:EOF$|is_mdef$|component$|no_range$|MOD_SYM$|MOD_SEP$|pure_elem$|countable$|ANY_SUFFIX$|FOLLOW_UP$)')
    ...
    char_range_heuristics = Series(NegativeLookahead(Alternative(RegExp('[\\n]'), more_than_one_blank, Series(dwsp__, literal_heuristics), Series(dwsp__, Option(Alternative(Text("::"), Text(":?"), Text(":"))), STRICT_SYM_REGEX, RegExp('\\s*\\]')))), Lookahead(Series(OneOrMore(range_desc), Text("]"))))
    ...
    char_range = Series(Text("["), Lookahead(char_range_heuristics), Option(Text("^")), OneOrMore(range_desc), SmartRE(fr"(?P<:Text>\])(?:{WSP_RE__})", "\"]\""))
    ...
    parser = Series(SmartRE(fr"(?P<:Text>@)(?:{WSP_RE__})", "\"@\""), name, SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), Option(argument), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""))
    ...
    ANY_SUFFIX = RegExp('[?*+]')
    is_mdef = Series(SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, Option(Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), placeholder, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), placeholder)), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""))), dwsp__, Retrieve(DEF))
    ...
    sequence = Series(Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround), ZeroOrMore(Series(NegativeLookahead(Text("@")), NegativeLookahead(Series(symbol, Retrieve(DEF))), Retrieve(AND), dwsp__, Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround))))
    ...
    macrodef = Series(Option(modifier), SmartRE(fr"(?P<:Text>\$)(?:{WSP_RE__})", "\"$\""), name, dwsp__, Option(Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), placeholder, ZeroOrMore(Series(SmartRE(fr"(?P<:Text>,)(?:{WSP_RE__})", "\",\""), placeholder)), SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\""), mandatory=1)), Retrieve(DEF), dwsp__, Option(Series(OR, dwsp__)), macrobody, Option(Series(MOD_SYM, dwsp__, hide)), Retrieve(ENDL), dwsp__, Lookahead(FOLLOW_UP))
    _is_def = Alternative(Series(Option(Series(MOD_SEP, symbol)), _DEF), Series(MOD_SEP, is_mdef))
    component = Alternative(regexp, literals, procedure, Series(symbol, NegativeLookahead(_DEF), NegativeLookahead(_is_def)), Series(Lookahead(Text("$")), NegativeLookahead(is_mdef), placeholder, NegativeLookahead(is_def), mandatory=2), Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), expression, SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\"")), Series(RAISE_EXPR_WO_BRACKETS, expression))
