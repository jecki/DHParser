...
class FlexibleEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "ae1983996fdc533434d38b164fc1910e"
    disposable__ = re.compile('(?:MOD_SEP$|ANY_SUFFIX$|FOLLOW_UP$|component$|is_mdef$|EOF$|pure_elem$|MOD_SYM$|countable$|no_range$)')
    ...
    sequence = Series(Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround), ZeroOrMore(Series(SmartRE(f'(?!@)', '!`@`'), NegativeLookahead(Series(symbol, Retrieve(DEF))), Retrieve(AND), dwsp__, Option(SmartRE(fr"(?P<:Text>§)(?:{WSP_RE__})", "\"§\"")), Alternative(interleave, lookaround))))
    ...
    component = Alternative(regexp, literals, procedure, Series(symbol, NegativeLookahead(_DEF), NegativeLookahead(_is_def)), Series(SmartRE(f'(?=\\$)', '&`$`'), NegativeLookahead(is_mdef), placeholder, NegativeLookahead(is_def), mandatory=2), Series(SmartRE(fr"(?P<:Text>\()(?:{WSP_RE__})", "\"(\""), expression, SmartRE(fr"(?P<:Text>\))(?:{WSP_RE__})", "\")\"")), Series(RAISE_EXPR_WO_BRACKETS, expression))
