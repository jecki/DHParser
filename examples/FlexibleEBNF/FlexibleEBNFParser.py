...
class FlexibleEBNFGrammar(Grammar):
    ...
    RE_CORE = RegExp('(?:(?<!\\\\)\\\\(?:/)|[^/])*')
    regex_heuristics = Series(NegativeLookahead(SmartRE('( +`[^`]*` +/| +´[^´]*´ +/| +\'[^\']*\' +/| +"[^"]*" +/| +\\w+ +/)', '/ +`[^`]*` +\\//|/ +´[^´]*´ +\\//|/ +\'[^\']*\' +\\//|/ +"[^"]*" +\\//|/ +\\w+ +\\//')), SmartRE('([^/\\n*?+\\\\]*[*?+\\\\][^/\\n]*/|[^\\w]+/|[^ ])', '/[^\\/\\n*?+\\\\]*[*?+\\\\][^\\/\\n]*\\//|/[^\\w]+\\//|/[^ ]/'))
    literal_heuristics = SmartRE('((?:~?\\s*"(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^"]*)*")|(?:~?\\s*\'(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^\']*)*\')|(?:~?\\s*`(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^`]*)*`)|(?:~?\\s*´(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^´]*)*´)|(?:~?\\s*/(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^/]*)*/))', '/~?\\s*"(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^"]*)*"/|/~?\\s*\'(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^\']*)*\'/|/~?\\s*`(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^`]*)*`/|/~?\\s*´(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^´]*)*´/|/~?\\s*\\/(?:[\\\\]\\]|[^\\]]|[^\\\\]\\[[^\\/]*)*\\//')
    more_than_one_blank = RegExp('[^ \\]]*[ ][^ \\]]*[ ]')
    STRICT_SYM_REGEX = RegExp('(?!\\d)\\w+')
    CH_LEADIN = Capture(SmartRE('(?P<:Text>0x|%x|U\\+|u\\+|\\#x|\\\\x|\\\\u|\\\\U)', '`0x`|`%x`|`U+`|`u+`|`#x`|`\\x`|`\\u`|`\\U`'), zero_length_warning=False)
    MOD_SYM = Drop(Text("->"))
    ...
    BRACE_SIGN = Capture(SmartRE('(?P<:Text>\\{|\\()', '`{`|`(`'), zero_length_warning=False)
    RNG_BRACE = Capture(Retrieve(BRACE_SIGN), zero_length_warning=True)
    ENDL = Capture(SmartRE('(?P<:Text>;|)', '`;`|``'), zero_length_warning=False)
    AND = Capture(SmartRE('(?P<:Text>,|)', '`,`|``'), zero_length_warning=False)
    ...
    char_range_heuristics = Series(NegativeLookahead(Alternative(RegExp('[\\n]'), more_than_one_blank, Series(dwsp__, literal_heuristics), Series(dwsp__, Option(SmartRE('(?P<:Text>::|:\\?|:)', '`::`|`:?`|`:`')), STRICT_SYM_REGEX, RegExp('\\s*\\]')))), Lookahead(Series(OneOrMore(range_desc), Text("]"))))
    ...
    retrieveop = Series(SmartRE('(?P<:Text>::|:\\?|:)', '"::"|":?"|":"'), dwsp__)
    flowmarker = Series(SmartRE('(?P<:Text>!|\\&|<\\-!|<\\-\\&)', '"!"|"&"|"<-!"|"<-&"'), dwsp__)
    ANY_SUFFIX = RegExp('[?*+]')
    ...
    MOD_SEP = RegExp(' *: *')
    hide = Series(SmartRE('(?P<:Text>HIDE|Hide|hide|DISPOSE|Dispose|dispose)', '"HIDE"|"Hide"|"hide"|"DISPOSE"|"Dispose"|"dispose"'), dwsp__)
    drop = Series(SmartRE('(?P<:Text>DROP|Drop|drop|SKIP|Skip|skip)', '"DROP"|"Drop"|"drop"|"SKIP"|"Skip"|"skip"'), dwsp__)
