...
class FlexibleEBNFGrammar(Grammar):
    ...
    element = Forward()
    expression = Forward()
    source_hash__ = "a38ec1373f4fe99cd948b27b10164af7"
    disposable__ = re.compile('(?:..(?<=^))|(?:MOD_SYM$|pure_elem$|no_range$|FOLLOW_UP$|EOF$|countable$|MOD_SEP$|is_mdef$|component$|ANY_SUFFIX$)')
    ...
    is_def = Series(Option(Series(MOD_SEP, symbol)), Retrieve(DEF))
    ...
    _is_def = Series(Option(Series(MOD_SEP, symbol)), _DEF)
