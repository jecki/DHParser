...
RE_COMMENT = NEVER_MATCH_PATTERN  # THIS MUST ALWAYS BE THE SAME AS InterwovenLR2Grammar.COMMENT__ !!!


def InterwovenLR2Tokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    InterwovenLR2Tokenizer, RE_INCLUDE, RE_COMMENT)
...
class InterwovenLR2Grammar(Grammar):
    r"""Parser for an InterwovenLR2 document.
    ...
    code as the single argument in order to use the parser, e.g.:
        parser = InterwovenLR2()
        syntax_tree = parser(source_code)
    """
    L = Forward()
    P = Forward()
    source_hash__ = "79b873b04321bc86ac08483a10687b25"
...
parsing: PseudoJunction = create_parser_junction(InterwovenLR2Grammar)
get_grammar = parsing.factory  # for backwards compatibility, only


try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        RE_COMMENT in (InterwovenLR2Grammar.COMMENT__, NEVER_MATCH_PATTERN), \
        "Please adjust the pre-processor-variable RE_COMMENT in file InterwovenLR2Parser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class InterwovenLR2Grammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, InterwovenLR2Grammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==InterwovenLR2Grammar.COMMENT__ allows the " \
...
InterwovenLR2_AST_transformation_table = {
    # AST Transformations for the InterwovenLR2-grammar
    # Special rules:
...
#     InterwovenLR2_AST_transformation_table, "CST", "AST", "transtable")

def InterwovenLR2Transformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=InterwovenLR2_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(InterwovenLR2Transformer), 'AST')
...
class InterwovenLR2Compiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        InterwovenLR2 source file.
    """

    def __init__(self):
        super(InterwovenLR2Compiler, self).__init__()
...
compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(InterwovenLR2Compiler), 'InterwovenLR2')
...
# # ("InterwovenLR2") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "InterwovenLR2", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "InterwovenLR2",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('InterwovenLR2_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from InterwovenLR2Parser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "InterwovenLR2"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " InterwovenLR2 file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('InterwovenLR2_serializations', serializations, allow_new_key=True)
        finalize_presets()
