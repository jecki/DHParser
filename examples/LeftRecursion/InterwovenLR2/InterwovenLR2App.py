...
"""InterwovenLR2App.py - a simple GUI for the compilation of InterwovenLR2-files"""
...
class InterwovenLR2App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('InterwovenLR2 App')
        ...
        grammar = InterwovenLR2Parser.parsing.factory()
        ...
        # self.targets = pp_paths(InterwovenLR2Parser.junctions, vertical='┊', bifurcation='├').split('\n')[1:]

        # # 2. show the complete pipeline as an indented tree
        # self.targets = str(as_graph(InterwovenLR2Parser.junctions)).split('\n')[1:]

        # 3. show only final targets
        self.targets = [j.dst for j in InterwovenLR2Parser.junctions]
        self.targets.sort(key=lambda s: s in InterwovenLR2Parser.targets)

        if len(self.targets) > 1:  self.targets.append(ALL_TARGETS_SPECIAL)
        self.compilation_target = list(InterwovenLR2Parser.targets)[0]
    ...
    def compile_single_unit(self, source, target, parser):
        if target == ALL_TARGETS_SPECIAL:
            path = InterwovenLR2Parser.junctions
        else:
            path = connection(InterwovenLR2Parser.junctions, target, "CST")
        ...
        try:
            results = InterwovenLR2Parser.pipeline(
    ...
    def on_compile(self):
        ...
        # self.all_results = InterwovenLR2Parser.pipeline(source, self.compilation_target, parser)
        # self.finish_single_unit()
        self.cancel_event.clear()
        self.worker = threading.Thread(
    ...
    def write_or_update_config_file(self, path, config) -> bool:
        ...
        ts2p_new = 'InterwovenLR2' not in config.sections()
        if ts2p_new:
            config['InterwovenLR2'] = {}
        cfg = get_config_values('InterwovenLR2.*')
        i = len('InterwovenLR2.')
        cfg = {k[i:]: str(v) for k, v in cfg.items()}
        config['InterwovenLR2'].update(cfg)
        try:
    ...
    def on_export_test(self):
        ...
        if ftype == 'config':
            ...
            if ftype == 'test':
                stages = (UNIT_STAGES
                    | frozenset(j.dst for j in InterwovenLR2Parser.junctions))
            ...
            if error_level < ERROR:
                for stage, result in self.all_results.items():
                    ...
                    tests[stage] = cases
            unit = { parser: tests }
            cfg_data = dump_config_data('InterwovenLR2.*', use_headings=False)
    ...
    def on_about(self):
        ...
        tk.messagebox.showinfo(
            title="About InterwovenLR2",
            message=("InterwovenLR2 was brought to you by:\n\n"
...
if __name__ == '__main__':
    ...
    if not InterwovenLR2Parser.main(called_from_app=True):
        app = InterwovenLR2App()
        app.mainloop()
