...
"""direct_hiddenServer.py - starts a server (if not already running) for the
                    compilation of direct_hidden
"""
...
class direct_hiddenCPUBoundTasks:
...
class direct_hiddenBlockingTasks:
    def __init__(self, lsp_data: dict):
        from DHParser.lsp import gen_lsp_table
        self.lsp_data = lsp_data
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
...
class direct_hiddenLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "direct_hidden-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = direct_hiddenCPUBoundTasks(self.lsp_data)
        self.blocking = direct_hiddenBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from direct_hiddenParser import batch_process
        args = argstr.split(' ')
    ...
    #     from direct_hiddenParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new direct_hiddenServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('direct_hidden_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    direct_hidden_lsp = direct_hiddenLanguageServerProtocol()
    lsp_table = direct_hidden_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', direct_hidden_lsp.simply_compile)
    direct_hidden_server = Server(rpc_functions=lsp_table,
                        cpu_bound=direct_hidden_lsp.cpu_bound.lsp_table.keys(),
                        blocking=direct_hidden_lsp.blocking.lsp_table.keys(),
                        connection_callback=direct_hidden_lsp.connect,
                        server_name="direct_hiddenServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        direct_hidden_server.echo_log = True if port >= 0 and host else False
        msg = direct_hidden_server.start_logging(log_path.strip('" \''))
        if direct_hidden_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        direct_hidden_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            direct_hidden_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing direct_hidden-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python direct_hiddenServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python direct_hiddenServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python direct_hiddenServer.py --stream\n'
             + '    python direct_hiddenServer.py --stopserver\n'
             + '    python direct_hiddenServer.py --status\n'
             + '    python direct_hiddenServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python direct_hiddenServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
