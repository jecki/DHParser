...
"""direct_hiddenApp.py - a simple GUI for the compilation of direct_hidden-files"""
...
class direct_hiddenApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('direct_hidden App')
        ...
        grammar = direct_hiddenParser.parsing.factory()
        ...
        # self.targets = pp_paths(direct_hiddenParser.junctions, vertical='┊', bifurcation='├').split('\n')[1:]

        # # 2. show the complete pipeline as an indented tree
        # self.targets = str(as_graph(direct_hiddenParser.junctions)).split('\n')[1:]

        # 3. show only final targets
        self.targets = [j.dst for j in direct_hiddenParser.junctions]
        self.targets.sort(key=lambda s: s in direct_hiddenParser.targets)

        if len(self.targets) > 1:  self.targets.append(ALL_TARGETS_SPECIAL)
        self.compilation_target = list(direct_hiddenParser.targets)[0]
            ...
            if self.names:
                ...
                if len(self.names) == 1:
                    ...
                    self.worker = threading.Thread(
                        target = direct_hiddenParser.batch_process,
    ...
    def compile_single_unit(self, source, target, parser):
        if target == ALL_TARGETS_SPECIAL:
            path = direct_hiddenParser.junctions
        else:
            path = connection(direct_hiddenParser.junctions, target, "CST")
        ...
        try:
            results = direct_hiddenParser.pipeline(
    ...
    def on_compile(self):
        ...
        # self.all_results = direct_hiddenParser.pipeline(source, self.compilation_target, parser)
        # self.finish_single_unit()
        self.cancel_event.clear()
        self.worker = threading.Thread(
    ...
    def write_or_update_config_file(self, path, config) -> bool:
        ...
        ts2p_new = 'direct_hidden' not in config.sections()
        if ts2p_new:
            config['direct_hidden'] = {}
        cfg = get_config_values('direct_hidden.*')
        i = len('direct_hidden.')
        cfg = {k[i:]: str(v) for k, v in cfg.items()}
        config['direct_hidden'].update(cfg)
        try:
    ...
    def on_export_test(self):
        ...
        if ftype == 'config':
            ...
            if ftype == 'test':
                stages = (UNIT_STAGES
                    | frozenset(j.dst for j in direct_hiddenParser.junctions))
            ...
            if error_level < ERROR:
                for stage, result in self.all_results.items():
                    ...
                    tests[stage] = cases
            unit = { parser: tests }
            cfg_data = dump_config_data('direct_hidden.*', use_headings=False)
    ...
    def on_about(self):
        ...
        tk.messagebox.showinfo(
            title="About direct_hidden",
            message=("direct_hidden was brought to you by:\n\n"
...
if __name__ == '__main__':
    ...
    if not direct_hiddenParser.main(called_from_app=True):
        app = direct_hiddenApp()
        app.mainloop()
