...
"""LRServer.py - starts a server (if not already running) for the
                    compilation of LR
"""
...
class LRLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "LR-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = LRCPUBoundTasks(self.lsp_data)
        self.blocking = LRBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from LRParser import batch_process
        args = argstr.split(' ')
    ...
    #     from LRParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new LRServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('LR_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    LR_lsp = LRLanguageServerProtocol()
    lsp_table = LR_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', LR_lsp.simply_compile)
    LR_server = Server(rpc_functions=lsp_table,
                        cpu_bound=LR_lsp.cpu_bound.lsp_table.keys(),
                        blocking=LR_lsp.blocking.lsp_table.keys(),
                        connection_callback=LR_lsp.connect,
                        server_name="LRServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        LR_server.echo_log = True if port >= 0 and host else False
        msg = LR_server.start_logging(log_path.strip('" \''))
        if LR_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        LR_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            LR_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing LR-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python LRServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python LRServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python LRServer.py --stream\n'
             + '    python LRServer.py --stopserver\n'
             + '    python LRServer.py --status\n'
             + '    python LRServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python LRServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
