...
"""LR2Server.py - starts a server (if not already running) for the
                    compilation of LR2
"""
...
class LR2LanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "LR2-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = LR2CPUBoundTasks(self.lsp_data)
        self.blocking = LR2BlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from LR2Parser import batch_process
        args = argstr.split(' ')
    ...
    #     from LR2Parser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new LR2Server. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('LR2_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    LR2_lsp = LR2LanguageServerProtocol()
    lsp_table = LR2_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', LR2_lsp.simply_compile)
    LR2_server = Server(rpc_functions=lsp_table,
                        cpu_bound=LR2_lsp.cpu_bound.lsp_table.keys(),
                        blocking=LR2_lsp.blocking.lsp_table.keys(),
                        connection_callback=LR2_lsp.connect,
                        server_name="LR2Server",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        LR2_server.echo_log = True if port >= 0 and host else False
        msg = LR2_server.start_logging(log_path.strip('" \''))
        if LR2_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        LR2_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            LR2_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing LR2-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python LR2Server.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python LR2Server.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python LR2Server.py --stream\n'
             + '    python LR2Server.py --stopserver\n'
             + '    python LR2Server.py --status\n'
             + '    python LR2Server.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python LR2Server.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
