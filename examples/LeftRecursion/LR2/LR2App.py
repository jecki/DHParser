...
"""LR2App.py - a simple GUI for the compilation of LR2-files"""
...
class LR2App(tk.Tk):
    def __init__(self):
        ...
        grammar = LR2Parser.parsing.factory()
        ...
        # self.targets = pp_paths(LR2Parser.junctions, vertical='┊', bifurcation='├').split('\n')[1:]

        # # 2. show the complete pipeline as an indented tree
        # self.targets = str(as_graph(LR2Parser.junctions)).split('\n')[1:]

        # 3. show only final targets
        self.targets = [j.dst for j in LR2Parser.junctions]
        self.targets.sort(key=lambda s: s in LR2Parser.targets)

        if len(self.targets) > 1:  self.targets.append(ALL_TARGETS_SPECIAL)
        self.compilation_target = list(LR2Parser.targets)[0]
            ...
            if self.names:
                ...
                if len(self.names) == 1:
                    ...
                    self.worker = threading.Thread(
                        target = LR2Parser.batch_process,
    ...
    def compile_single_unit(self, source, target, parser):
        if target == ALL_TARGETS_SPECIAL:
            path = LR2Parser.junctions
        else:
            path = connection(LR2Parser.junctions, target, "CST")
    ...
    def on_compile(self):
        ...
        # self.all_results = LR2Parser.pipeline(source, self.compilation_target, parser)
        # self.finish_single_unit()
        self.cancel_event.clear()
        self.worker = threading.Thread(
    ...
    def write_or_update_config_file(self, path, config) -> bool:
        ...
        ts2p_new = 'LR2' not in config.sections()
        if ts2p_new:
            config['LR2'] = {}
        cfg = get_config_values('LR2.*')
        i = len('LR2.')
        cfg = {k[i:]: str(v) for k, v in cfg.items()}
        config['LR2'].update(cfg)
        try:
    ...
    def on_export_test(self):
        ...
        if ftype == 'config':
            ...
            if ftype == 'test':
                stages = (UNIT_STAGES
                    | frozenset(j.dst for j in LR2Parser.junctions))
            ...
            if error_level < ERROR:
                for stage, result in self.all_results.items():
                    ...
                    tests[stage] = cases
            unit = { parser: tests }
            cfg_data = dump_config_data('LR2.*', use_headings=False)
    ...
    def on_about(self):
        ...
        tk.messagebox.showinfo(
            title="About LR2",
            message=("LR2 was brought to you by:\n\n"
...
if __name__ == '__main__':
    ...
    if not LR2Parser.main(called_from_app=True):
        app = LR2App()
        app.mainloop()
