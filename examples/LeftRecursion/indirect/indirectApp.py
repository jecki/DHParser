...
"""indirectApp.py - a simple GUI for the compilation of indirect-files"""
...
class indirectApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('indirect App')
        ...
        grammar = indirectParser.parsing.factory()
        ...
        # self.targets = pp_paths(indirectParser.junctions, vertical='┊', bifurcation='├').split('\n')[1:]

        # # 2. show the complete pipeline as an indented tree
        # self.targets = str(as_graph(indirectParser.junctions)).split('\n')[1:]

        # 3. show only final targets
        self.targets = [j.dst for j in indirectParser.junctions]
        self.targets.sort(key=lambda s: s in indirectParser.targets)

        if len(self.targets) > 1:  self.targets.append(ALL_TARGETS_SPECIAL)
        self.compilation_target = list(indirectParser.targets)[0]
            ...
            if self.names:
                ...
                if len(self.names) == 1:
                    ...
                    self.worker = threading.Thread(
                        target = indirectParser.batch_process,
    ...
    def compile_single_unit(self, source, target, parser):
        if target == ALL_TARGETS_SPECIAL:
            path = indirectParser.junctions
        else:
            path = connection(indirectParser.junctions, target, "CST")
        ...
        try:
            results = indirectParser.pipeline(
    ...
    def on_compile(self):
        ...
        # self.all_results = indirectParser.pipeline(source, self.compilation_target, parser)
        # self.finish_single_unit()
        self.cancel_event.clear()
        self.worker = threading.Thread(
    ...
    def write_or_update_config_file(self, path, config) -> bool:
        ...
        ts2p_new = 'indirect' not in config.sections()
        if ts2p_new:
            config['indirect'] = {}
        cfg = get_config_values('indirect.*')
        i = len('indirect.')
        cfg = {k[i:]: str(v) for k, v in cfg.items()}
        config['indirect'].update(cfg)
        try:
    ...
    def on_export_test(self):
        ...
        if ftype == 'config':
            ...
            if ftype == 'test':
                stages = (UNIT_STAGES
                    | frozenset(j.dst for j in indirectParser.junctions))
            ...
            if error_level < ERROR:
                for stage, result in self.all_results.items():
                    ...
                    tests[stage] = cases
            unit = { parser: tests }
            cfg_data = dump_config_data('indirect.*', use_headings=False)
    ...
    def on_about(self):
        ...
        tk.messagebox.showinfo(
            title="About indirect",
            message=("indirect was brought to you by:\n\n"
...
if __name__ == '__main__':
    ...
    if not indirectParser.main(called_from_app=True):
        app = indirectApp()
        app.mainloop()
