...
read_local_config(os.path.join(scriptdir, 'indirectConfig.ini'))
...
RE_COMMENT = NEVER_MATCH_PATTERN  # THIS MUST ALWAYS BE THE SAME AS indirectGrammar.COMMENT__ !!!


def indirectTokenizer(original_text) -> Tuple[str, List[Error]]:
    # Here, a function body can be filled in that adds preprocessor tokens
    # to the source code and returns the modified source.
    return original_text, []

preprocessing: PseudoJunction = create_preprocess_junction(
    indirectTokenizer, RE_INCLUDE, RE_COMMENT)
...
class indirectGrammar(Grammar):
    r"""Parser for an indirect document.
    ...
    """
    A = Forward()
    source_hash__ = "30c8f6462426f7f611fc94c55508a295"
    ...
    B = Alternative(Series(Ref("A"), Text("y")), Series(Ref("A"), Text("x")))
    A.set(Alternative(B, Text("x")))
    root__ = A
    
parsing: PseudoJunction = create_parser_junction(indirectGrammar)
get_grammar = parsing.factory  # for backwards compatibility, only


try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        RE_COMMENT in (indirectGrammar.COMMENT__, NEVER_MATCH_PATTERN), \
        "Please adjust the pre-processor-variable RE_COMMENT in file indirectParser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class indirectGrammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, indirectGrammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==indirectGrammar.COMMENT__ allows the " \
...
indirect_AST_transformation_table = {
    # AST Transformations for the indirect-grammar
    # Special rules:
...
#     indirect_AST_transformation_table, "CST", "AST", "transtable")

def indirectTransformer() -> TransformerFunc:
    return static(partial(
        transformer, 
        transformation_table=indirect_AST_transformation_table.copy(),
        src_stage='CST', 
        dst_stage='AST'))

ASTTransformation: Junction = Junction(
    'CST', ThreadLocalSingletonFactory(indirectTransformer), 'AST')
...
class indirectCompiler(Compiler):
    """Compiler for the abstract-syntax-tree of a 
        indirect source file.
    """

    def __init__(self):
        super(indirectCompiler, self).__init__()
...
compiling: Junction = Junction(
    'AST', ThreadLocalSingletonFactory(indirectCompiler), 'indirect')
...
# # ("indirect") in this example must be the name of some earlier stage, though.
# postprocessing: Junction = Junction(
#     "indirect", ThreadLocalSingletonFactory(PostProcessing), "refined")
#
...
def pipeline(source: str,
             target: Union[str, Set[str]] = "indirect",
...
def process_file(source: str, out_dir: str = '', target_set: Set[str]=frozenset(),
    ...
    # serializations = get_config_value('indirect_serializations', serializations)
...
def batch_process(file_names: List[str], out_dir: str,
    ...
    messages files.
    """
    from indirectParser import process_file_wrapper
...
def main(called_from_app=False) -> bool:
    ...
    a = "an" if "indirect"[0:1] in "AEIOUaeiou" else "a"
    parser = ArgumentParser(description="Parses " + a + " indirect file and shows its syntax-tree."
    ...
    if args.serialize:
        ...
        access_presets()
        set_preset_value('indirect_serializations', serializations, allow_new_key=True)
        finalize_presets()
