...
"""indirectServer.py - starts a server (if not already running) for the
...
class indirectLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "indirect-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = indirectCPUBoundTasks(self.lsp_data)
        self.blocking = indirectBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from indirectParser import batch_process
        args = argstr.split(' ')
    ...
    #     from indirectParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new indirectServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('indirect_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    indirect_lsp = indirectLanguageServerProtocol()
    lsp_table = indirect_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', indirect_lsp.simply_compile)
    indirect_server = Server(rpc_functions=lsp_table,
                        cpu_bound=indirect_lsp.cpu_bound.lsp_table.keys(),
                        blocking=indirect_lsp.blocking.lsp_table.keys(),
                        connection_callback=indirect_lsp.connect,
                        server_name="indirectServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        indirect_server.echo_log = True if port >= 0 and host else False
        msg = indirect_server.start_logging(log_path.strip('" \''))
        if indirect_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        indirect_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            indirect_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing indirect-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python indirectServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python indirectServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python indirectServer.py --stream\n'
             + '    python indirectServer.py --stopserver\n'
             + '    python indirectServer.py --status\n'
             + '    python indirectServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python indirectServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
