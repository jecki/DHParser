...
class jsonLanguageServerProtocol:
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "json-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = jsonCPUBoundTasks(self.lsp_data)
        self.blocking = jsonBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from jsonParser import batch_process
        args = argstr.split(' ')
    ...
    #     from jsonParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new jsonServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('json_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    lsp_table = json_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', json_lsp.simply_compile)
    json_server = Server(rpc_functions=lsp_table,
                        cpu_bound=json_lsp.cpu_bound.lsp_table.keys(),
                        blocking=json_lsp.blocking.lsp_table.keys(),
                        connection_callback=json_lsp.connect,
                        server_name="jsonServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        json_server.echo_log = True if port >= 0 and host else False
        msg = json_server.start_logging(log_path.strip('" \''))
        if json_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        json_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            json_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing json-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python jsonServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python jsonServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python jsonServer.py --stream\n'
             + '    python jsonServer.py --stopserver\n'
             + '    python jsonServer.py --status\n'
             + '    python jsonServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python jsonServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
