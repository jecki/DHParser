...
"""jsonApp.py - a simple GUI for the compilation of json-files"""
...
class jsonApp(tkinter.Tk):
    def __init__(self):
        ...
        if 'html' in jsonParser.targets or 'HTML' in jsonParser.targets:
            self.target = 'html'
        elif len(jsonParser.targets) == 1:
            self.target = list(jsonParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = jsonParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not jsonParser.main(called_from_app=True):
        app = jsonApp()
        app.mainloop()
