...
"""HTMLApp.py - a simple GUI for the compilation of HTML-files"""
...
class HTMLApp(tkinter.Tk):
    def __init__(self):
        ...
        if 'html' in HTMLParser.targets or 'HTML' in HTMLParser.targets:
            self.target = 'html'
        elif len(HTMLParser.targets) == 1:
            self.target = list(HTMLParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = HTMLParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not HTMLParser.main(called_from_app=True):
        app = HTMLApp()
        app.mainloop()
