...
class HTMLGrammar(Grammar):
    ...
    source_hash__ = "dbb81be219dba52dbde06f1899ecdc17"
    early_tree_reduction__ = CombinedParser.MERGE_TREETOPS
    disposable__ = re.compile('(?:EOF$|Misc$|NameStartChar$|CData$|NameChars$|BOM$|tagContent$|PubidCharsSingleQuoted$|EncName$|VersionNum$|Reference$|prolog$|PubidChars$|CommentChars$)')
    ...
    PITarget = Series(SmartRE(f'(?i)(?![Xx][Mm][Ll])', '!/[Xx][Mm][Ll]/'), Name)
    ...
    SDDecl = SmartRE(f'(?i){WSP_RE__}standalone{WSP_RE__}={WSP_RE__}(?:(?:(?:\')(?P<:IgnoreCase>yes|no)(?:\'))|(?:(?:")(?P<:IgnoreCase>yes|no)(?:")))', '~ \'standalone\' ~ \'=\' ~ "\'" `yes`|`no` "\'"|\'"\' `yes`|`no` \'"\'')
...
try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        ...
        "Please adjust the pre-processor-variable RE_COMMENT in file HTMLParser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class HTMLGrammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, HTMLGrammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==HTMLGrammar.COMMENT__ allows the " \
