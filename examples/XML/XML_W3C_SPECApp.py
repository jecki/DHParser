...
"""XML_W3C_SPECApp.py - a simple GUI for the compilation of XML_W3C_SPEC-files"""
...
import XML_W3C_SPECParser


class XML_W3C_SPECApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('XML_W3C_SPEC App')
        ...
        if 'html' in XML_W3C_SPECParser.targets or 'HTML' in XML_W3C_SPECParser.targets:
            self.target = 'html'
        elif len(XML_W3C_SPECParser.targets) == 1:
            self.target = list(XML_W3C_SPECParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = XML_W3C_SPECParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not XML_W3C_SPECParser.main(called_from_app=True):
        app = XML_W3C_SPECApp()
        app.mainloop()
