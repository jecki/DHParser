...
"""XMLApp.py - a simple GUI for the compilation of XML-files"""
...
class XMLApp(tkinter.Tk):
    def __init__(self):
        ...
        if 'html' in XMLParser.targets or 'HTML' in XMLParser.targets:
            self.target = 'html'
        elif len(XMLParser.targets) == 1:
            self.target = list(XMLParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = XMLParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not XMLParser.main(called_from_app=True):
        app = XMLApp()
        app.mainloop()
