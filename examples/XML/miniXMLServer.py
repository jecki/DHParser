...
"""miniXMLServer.py - starts a server (if not already running) for the
                    compilation of miniXML
"""
...
class miniXMLLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "miniXML-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = miniXMLCPUBoundTasks(self.lsp_data)
        self.blocking = miniXMLBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from miniXMLParser import batch_process
        args = argstr.split(' ')
    ...
    #     from miniXMLParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new miniXMLServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('miniXML_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    miniXML_lsp = miniXMLLanguageServerProtocol()
    lsp_table = miniXML_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', miniXML_lsp.simply_compile)
    miniXML_server = Server(rpc_functions=lsp_table,
                        cpu_bound=miniXML_lsp.cpu_bound.lsp_table.keys(),
                        blocking=miniXML_lsp.blocking.lsp_table.keys(),
                        connection_callback=miniXML_lsp.connect,
                        server_name="miniXMLServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        miniXML_server.echo_log = True if port >= 0 and host else False
        msg = miniXML_server.start_logging(log_path.strip('" \''))
        if miniXML_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        miniXML_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            miniXML_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing miniXML-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python miniXMLServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python miniXMLServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python miniXMLServer.py --stream\n'
             + '    python miniXMLServer.py --stopserver\n'
             + '    python miniXMLServer.py --status\n'
             + '    python miniXMLServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python miniXMLServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
