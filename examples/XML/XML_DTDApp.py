...
"""XML_DTDApp.py - a simple GUI for the compilation of XML_DTD-files"""
...
import XML_DTDParser


class XML_DTDApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('XML_DTD App')
        ...
        if 'html' in XML_DTDParser.targets or 'HTML' in XML_DTDParser.targets:
            self.target = 'html'
        elif len(XML_DTDParser.targets) == 1:
            self.target = list(XML_DTDParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = XML_DTDParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not XML_DTDParser.main(called_from_app=True):
        app = XML_DTDApp()
        app.mainloop()
