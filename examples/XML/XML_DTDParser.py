...
from DHParser.toolkit import re, is_filename, load_if_file, cpu_count, RX_NEVER_MATCH, \
...
class XML_DTDGrammar(Grammar):
    ...
    source_hash__ = "b04e3bdc81bafd97393ac366e642cbb8"
    ...
    PIChars = RegExp('(?:(?!\\?>)(?:\\x09|\\x0A|\\x0D|[\\u0020-\\uD7FF]|[\\uE000-\\uFFFD]|[\\U0001000'
       '0-\\U0010FFFF]))+')
    IgnoreChars = RegExp('(?:(?!(?:<!\\[)|(?:\\]\\]>))(?:\\x09|\\x0A|\\x0D|[\\u0020-\\uD7FF]|[\\uE000-\\u'
       'FFFD]|[\\U00010000-\\U0010FFFF]))+')
    ...
    seq = Series(Drop(Text('(')), dwsp__, Ref("cp"), ZeroOrMore(Series(dwsp__, Drop(Text(',')), dwsp__, Ref("cp"))), dwsp__, Drop(Text(')')))
    ...
    children = Series(Alternative(Ref("choice"), seq), Option(Alternative(Drop(Text('?')), Drop(Text('*')), Drop(Text('+')))))
    ...
    ignoreSect = Series(Drop(Text('<![')), dwsp__, Drop(Text('IGNORE')), dwsp__, Drop(Text('[')), Ref("ignoreSectContents"), Drop(Text(']]>')))
    includeSect = Series(Drop(Text('<![')), dwsp__, Drop(Text('INCLUDE')), dwsp__, Drop(Text('[')), Ref("extSubsetDecl"), Drop(Text(']]>')))
    ...
    cp.set(Series(Alternative(Name, Ref("choice"), seq), Option(Alternative(Drop(Text('?')), Drop(Text('*')), Drop(Text('+'))))))
    choice.set(Series(Drop(Text('(')), dwsp__, OneOrMore(Series(dwsp__, Drop(Text('|')), dwsp__, Ref("cp"))), dwsp__, Drop(Text(')'))))
    ignoreSectContents.set(Series(IgnoreChars, ZeroOrMore(Series(Drop(Text('<![')), Ref("ignoreSectContents"), Drop(Text(']]>')), IgnoreChars))))
...
try:
    assert RE_INCLUDE == NEVER_MATCH_PATTERN or \
        ...
        "Please adjust the pre-processor-variable RE_COMMENT in file XML_DTDParser.py so that " \
        "it either is the NEVER_MATCH_PATTERN or has the same value as the COMMENT__-attribute " \
        "of the grammar class XML_DTDGrammar! " \
        'Currently, RE_COMMENT reads "%s" while COMMENT__ is "%s". ' \
        % (RE_COMMENT, XML_DTDGrammar.COMMENT__) + \
        "\n\nIf RE_COMMENT == NEVER_MATCH_PATTERN then includes will deliberately be " \
        "processed, otherwise RE_COMMENT==XML_DTDGrammar.COMMENT__ allows the " \
