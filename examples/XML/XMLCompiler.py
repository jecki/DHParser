...
class XMLGrammar(Grammar):
    ...
    CharRef = Alternative(Series(_Token('&#'), RegExp('[0-9]+'), _Token(';')), Series(_Token('&#x'), RegExp('[0-9a-fA-F]+'), _Token(';')))
    ...
    CDSect = Series(_Token('<![CDATA['), CData, _Token(']]>'))
    PITarget = Series(NegativeLookahead(RegExp('X|xM|mL|l')), Name)
    PI = Series(_Token('<?'), PITarget, Option(Series(whitespace__, PIChars)), _Token('?>'))
    Comment = Series(_Token('<!--'), ZeroOrMore(Alternative(CommentChars, RegExp('-(?!-)'))), _Token('-->'))
    ...
    PEReference = Series(_Token('%'), Name, _Token(';'))
    EntityRef = Series(_Token('&'), Name, _Token(';'))
    Reference = Alternative(EntityRef, CharRef)
    PubidLiteral = Alternative(Series(_Token('"'), Option(PubidChars), _Token('"')), Series(_Token("'"), Option(PubidCharsSingleQuoted), _Token("'")))
    SystemLiteral = Alternative(Series(_Token('"'), RegExp('[^"]*'), _Token('"')), Series(_Token("'"), RegExp("[^']*"), _Token("'")))
    AttValue = Alternative(Series(_Token('"'), ZeroOrMore(Alternative(RegExp('[^<&"]+'), Reference)), _Token('"')), Series(_Token("'"), ZeroOrMore(Alternative(RegExp("[^<&']+"), Reference)), _Token("'")))
    EntityValue = Alternative(Series(_Token('"'), ZeroOrMore(Alternative(RegExp('[^%&"]+'), PEReference, Reference)), _Token('"')), Series(_Token("'"), ZeroOrMore(Alternative(RegExp("[^%&']+"), PEReference, Reference)), _Token("'")))
    content = Series(Option(CharData), ZeroOrMore(Series(Alternative(element, Reference, CDSect, PI, Comment), Option(CharData))))
    Attribute = Series(Name, whitespace__, _Token('='), whitespace__, AttValue, mandatory=2)
    TagName = Capture(Name)
    emptyElement = Series(_Token('<'), Name, ZeroOrMore(Series(whitespace__, Attribute)), whitespace__, _Token('/>'))
    ETag = Series(_Token('</'), Pop(TagName), whitespace__, _Token('>'), mandatory=1)
    STag = Series(_Token('<'), TagName, ZeroOrMore(Series(whitespace__, Attribute)), whitespace__, _Token('>'))
    element.set(Alternative(emptyElement, Series(STag, content, ETag, mandatory=1)))
    NDataDecl = Series(_Token('NData'), S, Name, mandatory=1)
    PublicID = Series(_Token('PUBLIC'), S, PubidLiteral, mandatory=1)
    ExternalID = Series(_Token('SYSTEM'), S, SystemLiteral, mandatory=1)
    NotationDecl = Series(_Token('<!NOTATION'), S, Name, whitespace__, Alternative(ExternalID, PublicID), whitespace__, _Token('>'), mandatory=1)
    PEDef = Alternative(EntityValue, ExternalID)
    EntityDef = Alternative(EntityValue, Series(ExternalID, Option(NDataDecl)))
    PEDecl = Series(_Token('<!ENTITY'), S, _Token('%'), S, Name, S, PEDef, whitespace__, _Token('>'), mandatory=3)
    GEDecl = Series(_Token('<!ENTITY'), S, Name, S, EntityDef, whitespace__, _Token('>'), mandatory=3)
    EntityDecl = Alternative(GEDecl, PEDecl)
    FIXED = Series(Option(Series(_Token('#FIXED'), S)), AttValue)
    IMPLIED = _Token('#IMPLIED')
    REQUIRED = _Token('#REQUIRED')
    DefaultDecl = Alternative(REQUIRED, IMPLIED, FIXED)
    Enumeration = Series(_Token('('), whitespace__, Nmtoken, ZeroOrMore(Series(whitespace__, _Token('|'), whitespace__, Nmtoken)), whitespace__, _Token(')'))
    NotationType = Series(_Token('NOTATION'), S, _Token('('), whitespace__, Name, ZeroOrMore(Series(whitespace__, _Token('|'), whitespace__, Name)), whitespace__, _Token(')'))
    EnumeratedType = Alternative(NotationType, Enumeration)
    NMTOKENS = _Token('NMTOKENS')
    NMTOKEN = _Token('NMTOKEN')
    ENTITIES = _Token('ENTITIES')
    ENTITY = _Token('ENTITY')
    IDREFS = _Token('IDREFS')
    IDREF = _Token('IDREF')
    ID = _Token('ID')
    TokenizedType = Alternative(ID, IDREF, IDREFS, ENTITY, ENTITIES, NMTOKEN, NMTOKENS)
    StringType = _Token('CDATA')
    ...
    AttlistDecl = Series(_Token('<!ATTLIST'), S, Name, ZeroOrMore(Series(whitespace__, AttDef)), whitespace__, _Token('>'), mandatory=1)
    seq = Series(_Token('('), whitespace__, cp, ZeroOrMore(Series(whitespace__, _Token(','), whitespace__, cp)), whitespace__, _Token(')'))
    cp.set(Series(Alternative(Name, choice, seq), Option(Alternative(_Token('?'), _Token('*'), _Token('+')))))
    choice.set(Series(_Token('('), whitespace__, OneOrMore(Series(whitespace__, _Token('|'), whitespace__, cp)), whitespace__, _Token(')')))
    children = Series(Alternative(choice, seq), Option(Alternative(_Token('?'), _Token('*'), _Token('+'))))
    Mixed = Alternative(Series(_Token('('), whitespace__, _Token('#PCDATA'), ZeroOrMore(Series(whitespace__, _Token('|'), whitespace__, Name)), whitespace__, _Token(')*')), Series(_Token('('), whitespace__, _Token('#PCDATA'), whitespace__, _Token(')')))
    ANY = _Token('ANY')
    EMPTY = _Token('EMPTY')
    contentspec = Alternative(EMPTY, ANY, Mixed, children)
    elementdecl = Series(_Token('<!ELEMENT'), S, Name, whitespace__, contentspec, whitespace__, _Token('>'), mandatory=1)
    TextDecl = Series(_Token('<?xml'), Option(VersionInfo), EncodingDecl, whitespace__, _Token('?>'))
    extParsedEnt = Series(Option(TextDecl), content)
    ignoreSectContents.set(Series(IgnoreChars, ZeroOrMore(Series(_Token('<!['), ignoreSectContents, _Token(']]>'), IgnoreChars))))
    ignoreSect = Series(_Token('<!['), whitespace__, _Token('IGNORE'), whitespace__, _Token('['), ignoreSectContents, _Token(']]>'))
    includeSect = Series(_Token('<!['), whitespace__, _Token('INCLUDE'), whitespace__, _Token('['), extSubsetDecl, _Token(']]>'))
    ...
    doctypedecl = Series(_Token('<!DOCTYPE'), whitespace__, Name, Option(Series(whitespace__, ExternalID)), whitespace__, Option(Series(_Token('['), intSubset, _Token(']'), whitespace__)), _Token('>'))
    No = _Token('no')
    Yes = _Token('yes')
    SDDecl = Series(whitespace__, _Token('standalone'), whitespace__, _Token('='), whitespace__, Alternative(Alternative(Series(_Token("'"), Yes), Series(No, _Token("'"))), Alternative(Series(_Token('"'), Yes), Series(No, _Token('"')))))
    EncName = RegExp('[A-Za-z][A-Za-z0-9._\\-]*')
    EncodingDecl.set(Series(whitespace__, _Token('encoding'), whitespace__, _Token('='), whitespace__, Alternative(Series(_Token("'"), EncName, _Token("'")), Series(_Token('"'), EncName, _Token('"')))))
    VersionNum = RegExp('[0-9]+\\.[0-9]+')
    VersionInfo.set(Series(whitespace__, _Token('version'), whitespace__, _Token('='), whitespace__, Alternative(Series(_Token("'"), VersionNum, _Token("'")), Series(_Token('"'), VersionNum, _Token('"')))))
    XMLDecl = Series(_Token('<?xml'), VersionInfo, Option(EncodingDecl), Option(SDDecl), whitespace__, _Token('?>'))
