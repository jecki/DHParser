...
"""HTMLServer.py - starts a server (if not already running) for the
                    compilation of HTML
"""
...
class HTMLLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "HTML-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = HTMLCPUBoundTasks(self.lsp_data)
        self.blocking = HTMLBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from HTMLParser import batch_process
        args = argstr.split(' ')
    ...
    #     from HTMLParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new HTMLServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('HTML_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    HTML_lsp = HTMLLanguageServerProtocol()
    lsp_table = HTML_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', HTML_lsp.simply_compile)
    HTML_server = Server(rpc_functions=lsp_table,
                        cpu_bound=HTML_lsp.cpu_bound.lsp_table.keys(),
                        blocking=HTML_lsp.blocking.lsp_table.keys(),
                        connection_callback=HTML_lsp.connect,
                        server_name="HTMLServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        HTML_server.echo_log = True if port >= 0 and host else False
        msg = HTML_server.start_logging(log_path.strip('" \''))
        if HTML_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        HTML_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            HTML_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing HTML-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python HTMLServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python HTMLServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python HTMLServer.py --stream\n'
             + '    python HTMLServer.py --stopserver\n'
             + '    python HTMLServer.py --status\n'
             + '    python HTMLServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python HTMLServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
