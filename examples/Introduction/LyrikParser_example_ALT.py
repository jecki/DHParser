...
from DHParser import is_filename, Grammar, Compiler, Lookbehind, \
    Alternative, Pop, Text, Synonym, Whitespace, \
...
class LyrikGrammar(Grammar):
    ...
    verknüpfung = Series(Series(Text("<"), wsp__), ziel, Series(Text(">"), wsp__))
    ...
    werk = Series(wortfolge, Option(Series(Series(Text("."), wsp__), untertitel, mandatory=1)), Option(verknüpfung))
    autor = Series(namenfolge, Option(verknüpfung))
    bibliographisches = Series(autor, Series(Text(","), wsp__), Option(NZ), werk, Series(Text(","), wsp__),
                               Option(NZ), ort, Series(Text(","), wsp__), Option(NZ), jahr, Series(Text("."), wsp__), mandatory=1)
