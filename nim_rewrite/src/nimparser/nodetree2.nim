...
type
  ...
  NodeObj {.acyclic.} = object of RootObj
    name*: string
    children: seq[Node]
    text: Option[StringSlice]  # must be none if field children is not empty!
...
proc init*(node: Node, 
  ...
  when content is seq[Node]:
    ...
    node.text = none(StringSlice)
  else:
    node.children = @[]
    node.text = option(toStringSlice(content))
...
proc isEmpty*(node: Node): bool = node.children.len == 0 and not isSome(node.text) or node.text.get.len == 0

func isAnonymous*(node: Node): bool = node.name.len == 0 or node.name[0] == ':'

proc content*(node: Node): string =
  if node.isLeaf:
    if isSome(node.text): $(node.text.get)  else: return ""
  else:
...
proc `result=`*(node: Node, children: seq[Node]) =
  ...
  node.text = option[StringSlice](nil)

proc runeLen*(node: Node): int =
  result = 0
  if node.isLeaf:
    let text: StringSlice = node.text.get
    var i = text.start
    while i <= text.stop:
